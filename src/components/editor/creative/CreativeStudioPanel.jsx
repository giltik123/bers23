...
export default function CreativeStudioPanel({ project, objects, onApply, disabled }) {
  const [goalId, setGoalId] = useState('marketing'); const [ideaId, setIdeaId] = useState(null); const [outfits, setOutfits] = useState([]); const [saved, setSaved] = useState(null);
  useEffect(() => { outfitManager.list().then(setOutfits); }, []);
  ...
  const strategy = useMemo(() => creativeStudio.strategy({ idea, goal: generated.goal, analysis, outfits }), [idea, generated.goal, analysis, outfits]);
  ...
  return <div className="space-y-4 rounded-2xl border border-border/60 p-4"><div><p className="font-medium">Creative Studio</p><p className="text-xs text-muted-foreground">Plans ideas and execution strategies; it never edits your image directly.</p></div><CreativeGoalPicker value={goalId} onChange={selectGoal} /><div className="grid gap-2 md:grid-cols-3">{generated.ideas.map((item) => <CreativeIdeaCard key={item.id} idea={item} active={item.id === idea.id} onSelect={selectIdea} />)}</div><div><p className="mb-2 text-sm font-medium">Mood boards</p><div className="grid gap-2 md:grid-cols-2">{generated.moodBoards.map((board) => <CreativeMoodBoardCard key={board.id} board={board} />)}</div></div><div className="rounded-xl bg-secondary/50 p-3 text-xs text-muted-foreground">Suggested workspace: <span className="font-medium text-foreground">{strategy.recommendations.workspace}</span>{strategy.recommendations.outfits.length > 0 && <span> · Outfit ideas: {strategy.recommendations.outfits.map((outfit) => outfit.name).join(', ')}</span>}</div><CreativeStrategySummary strategy={strategy} saved={saved} onApply={(value) => { creativeAnalytics.track('strategy_applied', { goal: goalId }); onApply(value); }} onSave={save} onFavorite={favorite} disabled={disabled} /></div>;
}