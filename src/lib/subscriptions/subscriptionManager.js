...
class SubscriptionManager {
  ...
  async ensure() {
    ...
    let [subscription] = await coreClient.entities.UserSubscription.filter({ created_by_id: user.id });
    if (!subscription) {
      subscription = await coreClient.entities.UserSubscription.create({ plan_id: 'free', status: 'free', billing_interval: 'monthly', started_at: new Date().toISOString(), trial_used: false, metadata: {} });
    ...
    if (subscription.status === 'trialing' && !subscriptionPolicy.isTrialActive(subscription)) {
      ...
      subscription = await coreClient.entities.UserSubscription.update(subscription.id, { plan_id: 'free', status: 'expired' });
  ...
  async changePlan(planId, { interval = 'monthly', source = 'manual' } = {}) {
    ...
    const updated = await coreClient.entities.UserSubscription.update(subscription.id, { plan_id: planId, status: planId === 'free' ? 'free' : 'active', billing_interval: interval, renewal_date: null, cancelled_at: null });
  ...
  async startTrial(planId) {
    ...
    const updated = await coreClient.entities.UserSubscription.update(subscription.id, { plan_id: planId, status: 'trialing', trial_started_at: now.toISOString(), trial_ends_at: ends, trial_used: true });
