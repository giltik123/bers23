...
class SubscriptionUsageService {
  ...
  async ensure() {
    ...
    let [usage] = await base44.entities.SubscriptionUsage.filter({ created_by_id: user.id, period_key: key });
    if (!usage) usage = await base44.entities.SubscriptionUsage.create({ period_key: key, daily_usage: {}, monthly_usage: {}, feature_usage: {} });
    this.current = usage;
    return usage;
  }
  async track({ projects = 0, credits = 0, storage = 0, aiGenerations = 0, tryons = 0, recipes = 0, feature = null }) {
    ...
    this.current = await base44.entities.SubscriptionUsage.update(usage.id, {
