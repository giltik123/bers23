...
export const creativeAnalytics = {
  track(eventName, properties = {}) { const state = read(); state.events = [{ eventName, properties, at: Date.now() }, ...state.events].slice(0, 300); localStorage.setItem(KEY, JSON.stringify(state)); coreClient.analytics.track({ eventName: `creative_${eventName}`, properties }); },
