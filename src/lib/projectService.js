...
// Project Engine service layer — ALL authoritative Project mutations cross explicit Core boundaries.
export const projectService = {
  ...
  acceptFinal: (id, payload) => coreClient.projects.acceptFinal(id, payload),
...
export function sortProjects(projects, sortBy) {
  ...
  switch (sortBy) {
    ...
    case 'created': return list.sort((a, b) => new Date(b.created_date) - new Date(a.created_date));
    case 'edits': return list.sort((a, b) => (b.operations?.length || 0) - (a.operations?.length || 0));
    case 'recent':
    default: return list.sort((a, b) => new Date(b.updated_date) - new Date(a.updated_date));
  }
}
