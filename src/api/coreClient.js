...
const persistedToken = () => appParams.token || storage()?.getItem('core_access_token') || undefined;
...
export const coreClient = Object.freeze({
  auth: {
    ...
    loginViaEmailPassword: async (email, password) => {
      ...
      if (result?.access_token) storage()?.setItem('core_access_token', result.access_token);
      return result;
    },
    hasToken: () => Boolean(persistedToken()),
    getToken: () => persistedToken(),
    setToken: (token) => storage()?.setItem('core_access_token', token),
    clearToken: () => storage()?.removeItem('core_access_token'),
