...
export interface InteractiveSegmentationPort { segment(input: Readonly<{ requestId: string; imageArtifactId: string; analysis: AnalysisTransform; points: readonly PromptPoint[]; privacyMode: PrivacyMode }>): Promise<Readonly<{ target: 'LOCAL' | 'CLOUD'; modelId: string; modelVersion: string; latencyMs: number; candidates: readonly SelectionCandidate[] }>>; cancel(requestId: string): void }
export interface CanonicalMaskArtifactPort { persist(mask: OriginalMask, metadata: Readonly<Record<string, unknown>>): Promise<CreativeArtifact> }
