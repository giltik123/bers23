...
export default function Editor() {
  ...
  const acceptFinalRef = useRef();
  acceptFinalRef.current = acceptFinal;
  ...
  const detect = async () => {
    setDetecting(true); setAiError(null); setLastAction(() => detect);
    try {
      const result = await jobManager.submit({ type: 'segmentation', label: 'Detect objects', priority: 'high', projectId: project.id, provider: 'sam3', estimatedTime: 12000, run: () => segmentationService.start({ projectId, imageUrl: project.current_image_url }) });
      setSegMeta({ status: result.status, fromCache: result.fromCache }); await saveObjects(result.objects);
    } catch (e) { setAiError(e.message || 'Object detection failed'); }
  ...
  const applyEdit = async (bypassCache = false, { skipDriftCheck = false, instructionOverride = null } = {}) => {
    ...
    const usedPlan = instructionOverride ? aiPlanner.plan({ project, instruction: usedInstruction, objects, selectedObject: selected }) : plan;
    ...
    if (!skipDriftCheck && memory && styleLock.isEnabled(project.id)) {
      ...
      if (report.exceedsThreshold) { setDriftWarning(report); return; }
    }
    setApplying(true); setAiError(null); setLastAction(() => applyEdit);
    try {
      ...
      if (result.status !== 'SUCCESS' || !result.imageUrl || !result.finalArtifactId) throw Object.assign(new Error('Edit failed'), { code: 'provider_failure' });
      ...
      if (e.code !== 'cancelled') { setAiError(e.message || 'Edit failed'); recipeEngine.recordOutcome(activeRecipe?.id, { success: false, durationMs: 0, credits: 0 }); workspaceHistory.recordEdit(workspaceManager.activeId(), { success: false, durationMs: 0 }); }
  ...
  const acceptResult = async () => {
    ...
    try {
      ...
      if (!result.finalArtifactId) throw new Error('Canonical FINAL identity is unavailable');
      const accepted = await acceptFinal(result.finalArtifactId, used, { executionId: result.executionId, type: 'edit', creditsUsed: result.credits_used || 0 });
      ...
      sceneMemory.recordAcceptedEdit(accepted).catch((error) => console.error('[Editor] Failed to update scene memory', error));
      workspaceHistory.recordEdit(workspaceManager.activeId(), { success: true, durationMs: result.generation_time_ms || 0 });
      setPendingResult(null); setInstruction(''); setActiveRecipe(null);
    } finally { setCommitting(false); }
  };

  const commitCanonicalResult = async (result, label, type = 'edit') => {
    if (!result?.finalArtifactId) throw Object.assign(new Error('This legacy result has no canonical FINAL identity and cannot enter Project history'), { code: 'missing_final_artifact_id' });
    return acceptFinalRef.current(result.finalArtifactId, label, { executionId: result.executionId, type, creditsUsed: result.credits_used || 0 });
  };

  const runChain = async (chain) => {
    setApplying(true); setAiError(null); setChainState({ chain, steps: chain.steps.map((s) => ({ label: s.label, status: 'pending' })), running: true });
    try {
      await jobManager.submit({
        type: 'chain', label: chain.name, priority: 'normal', projectId: project.id, provider: 'reve', estimatedTime: 60000,
        creditsReserved: legacyRecipeExecutionAdapter.estimate(chain), onCancel: () => legacyRecipeExecutionAdapter.cancel(), notifyOnComplete: true,
        run: () => legacyRecipeExecutionAdapter.execute({
          chain, project, objects,
          onProgress: (steps) => setChainState((cs) => ({ ...cs, steps })),
          onStepCommitted: async (result, step) => { const accepted=await commitCanonicalResult(result, `${chain.name}: ${step.label}`, 'recipe'); sceneMemory.recordAcceptedEdit(accepted).catch((error) => console.error('[Editor] Failed to update scene memory', error)); },
        }),
      });
      setChainState((cs) => ({ ...cs, running: false }));
    } catch (e) { setChainState((cs) => (cs ? { ...cs, running: false } : cs)); if (e.code !== 'cancelled') setAiError(e.message || 'Chain failed'); else setChainState(null); }
    finally { setApplying(false); }
  };

  const retryResult = () => { setPendingResult(null); applyEdit(true, { skipDriftCheck: true }); };
  const handleRename = async () => { const name = window.prompt('Rename project', project.name); if (name && name !== project.name) await rename(name); };
  const handleCreateVersion = async () => { const name = window.prompt('Version name', `Version ${(project.versions?.length || 0) + 1}`); if (name) await createVersion(name); };
  ...
  if (error || !project) return <div className="max-w-xl mx-auto px-4 py-16"><ErrorBanner message={error || 'Project not found'} onRetry={projectId ? reload : null} /></div>;
    ...
    <AdaptiveLayout className="max-w-3xl">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1 min-w-0">
          ...
          <button onClick={handleRename} className="p-1.5 rounded-lg hover:bg-accent transition-colors text-muted-foreground" aria-label="Rename project"><Pencil className="w-3.5 h-3.5" /></button>
        </div>
        <AdaptiveToolbar>
          <HistoryControls canUndo={canUndo} canRedo={canRedo} disabled={applying || detecting} onUndo={undo} onRedo={redo} onRestore={restoreOriginal} />
          <VersionsPanel versions={project.versions || []} onCreate={handleCreateVersion} onRestore={restoreVersion} disabled={applying || detecting} />
          <a href={project.current_image_url} target="_blank" rel="noreferrer" className="p-2 rounded-lg hover:bg-accent transition-colors" aria-label="Download"><Download className="w-5 h-5" /></a>
        </AdaptiveToolbar>
      </div>
      ...
      {driftWarning && !pendingResult && (
        <ConsistencyWarning warnings={driftWarning.warnings} onCancel={() => setDriftWarning(null)} onContinue={() => { setDriftWarning(null); applyEdit(false, { skipDriftCheck: true }); }} onAutoCorrect={() => { const corrected = `${instruction}. ${driftWarning.warnings.map((w) => w.correction).join(' ')}`; setDriftWarning(null); setInstruction(corrected); applyEdit(false, { skipDriftCheck: true, instructionOverride: corrected }); }} />
      )}
      <AdaptivePanel title="Job Center"><JobQueuePanel /></AdaptivePanel>
      <GenerationProgress />
      {chainState && <ChainProgress chain={chainState.chain} steps={chainState.steps} running={chainState.running} onCancel={() => legacyRecipeExecutionAdapter.cancel()} onDismiss={() => setChainState(null)} />}
      <SegmentationProgress />
      <ImageCanvas imageUrl={project.current_image_url} objects={objects} selectedId={selected?.id} onSelect={(obj) => selectObject(obj.id)} busy={applying} onUndo={undo} onRedo={redo} selection={selection} onSelectionPointer={selectionPointer} />
      <SelectionToolbar selection={selection} brushSize={brushSize} onBrushSize={setBrushSize} onStart={startSelection} onMode={(mode) => updateSelection((service) => service.setMode(mode))} onUndo={() => updateSelection((service) => service.undo())} onRedo={() => updateSelection((service) => service.redo())} onClear={() => updateSelection((service) => service.clear())} onCancel={() => { selectionServiceRef.current.cancel(); selectionServiceRef.current = null; setSelection(null); }} onDone={finishSelection} />
      <PipelineStatusBar width={project.width} height={project.height} />
      <CreditsBar estimate={0} />
      <AdaptivePanel title="Scene Memory"><SceneMemoryPanel project={project} /></AdaptivePanel>
      <EditorStatusBar objectCount={objects.length} selectionCount={objects.filter((o) => o.selected).length} selectionMode="single" maskedCount={objects.filter((o) => o.mask_url).length} segmentationStatus={segMeta?.status || (objects.length ? 'completed' : 'idle')} cacheStatus={segMeta ? (segMeta.fromCache ? 'hit' : 'miss') : 'empty'} />
      {objects.length > 0 && <AdaptivePanel title="Objects"><ObjectPanel objects={objects} onSelect={(obj) => selectObject(obj.id)} /></AdaptivePanel>}

      {objects.length === 0 ? (
        <Button onClick={detect} disabled={detecting} className="w-full h-12 rounded-2xl text-base">{detecting ? <Loader2 className="w-5 h-5 mr-2 animate-spin" /> : <ScanSearch className="w-5 h-5 mr-2" />}{detecting ? 'Detecting objects…' : 'Detect objects'}</Button>
      ) : pendingResult ? (
        <ResultCompare beforeUrl={pendingResult.beforeUrl} result={pendingResult.result} onAccept={acceptResult} onDiscard={() => setPendingResult(null)} onRetry={retryResult} busy={committing} />
      ) : (
        <>
          <WorkspaceToolbar disabled={applying} onUse={(prompt) => { setInstruction(prompt); setActiveRecipe(null); setEditTab('prompt'); }} />
          <WorkspaceRecommendations disabled={applying} onUse={(prompt, recipe) => { setInstruction(prompt); setActiveRecipe(recipe); setEditTab('prompt'); }} />
          ...
          {editTab === 'creative' ? (
            ...
            <OutfitPanel project={project} objects={objects} onCommit={async (result, outfit) => { const accepted=await commitCanonicalResult(result, `Try-On: ${outfit.name}`, 'try_on'); sceneMemory.recordAcceptedEdit(accepted).catch((error) => console.error('[Editor] Failed to update scene memory', error)); workspaceHistory.recordEdit(workspaceManager.activeId(), { success: true, durationMs: result.generation_time_ms || 0 }); }} />
          ) : editTab === 'fashion' ? (
            <FashionPanel />
          ) : editTab === 'agent' ? (
            <AgentPanel project={project} objects={objects} disabled={applying} onCommit={async (result, task) => { const accepted=await commitCanonicalResult(result, `Agent: ${task.label}`, 'agent'); sceneMemory.recordAcceptedEdit(accepted).catch((error) => console.error('[Editor] Failed to update scene memory', error)); }} onRollback={() => undo()} />
          ) : editTab === 'recipes' ? (
            <RecipePanel objects={objects} selectedObjects={objects.filter((o) => o.selected)} onRunChain={runChain} disabled={applying} onUse={(prompt, recipe) => { setInstruction(prompt); setActiveRecipe(recipe); setEditTab('prompt'); }} />
          ) : (
            <>
              {activeRecipe && <p className="text-[11px] text-muted-foreground">Recipe: <span className="font-medium text-foreground">{activeRecipe.name}</span> — edit the prompt below if needed.</p>}
              {plan && <PlanPreview plan={plan} />}
              <InstructionBar selectedObject={selected} instruction={instruction} onInstructionChange={setInstruction} onApply={() => applyEdit(false)} applying={applying} />
            </>
          )}
          </Suspense>
        </>
