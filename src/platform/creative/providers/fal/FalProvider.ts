...
export class FalProvider implements CreativeProvider {
  ...
  async execute(request: ProviderRequest): Promise<ProviderResult> {
    ...
    } catch (cause) { this.failures += 1; this.lastFailure = this.dependencies.clock(); throw cause instanceof FalProviderError ? cause : this.errors.map(undefined, undefined, cause); }
  }
