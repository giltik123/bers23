...
export function validateCreativeOutcome(outcome: CreativeOutcome): ValidationResult {
  ...
  if (![outcome?.scope?.tenantId,outcome?.scope?.projectId,outcome?.scope?.userId].every(v=>typeof v==='string'&&v.trim().length>0)) reasons.push('scope:invalid');
  if (typeof outcome?.executionId !== 'string' || !outcome.executionId.trim()) reasons.push('executionId:invalid');
  ...
  if (!['NOT_BILLABLE','AUTHORIZED','COMMITTED','RELEASED'].includes(outcome?.billing?.state) || !finite(outcome?.billing?.actualCost) || outcome.billing.actualCost !== a?.actualCost) reasons.push('billing:inconsistent');
  if (!Array.isArray(outcome?.candidateSet) || !outcome.candidateSet.length || !outcome.candidateSet.some(c => c.candidateId === outcome.selectedCandidateId) || outcome.candidateSet.some(c=>!c.candidateId || Object.entries(c).some(([k,v])=>k!=='candidateId'&&typeof v==='number'&&!finite(v)))) reasons.push('candidates:invalid');
