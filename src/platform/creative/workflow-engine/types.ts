...
export type WorkflowOutputBinding = Readonly<{ logicalId: string; artifactId: string; kind: string; slot: number }>;
export type WorkflowOperation = Readonly<{ id: string; type: string; executionRoute?: 'PROVIDER' | 'INTERNAL' | 'ON_DEVICE'; dependencies?: readonly string[]; requiredArtifacts?: readonly string[]; produces?: readonly string[]; outputArtifacts?: readonly string[]; outputBindings?: readonly WorkflowOutputBinding[]; providerId?: string; alternativeType?: string; reusableArtifactId?: string; cost?: Partial<ResourceBudget>; permissions?: readonly string[]; policyTags?: readonly string[]; input?: Readonly<Record<string, unknown>> }>;
