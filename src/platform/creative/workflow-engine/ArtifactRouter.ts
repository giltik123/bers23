...
export class ArtifactRouter {
  ...
  put(artifact: Artifact): Artifact { const frozen = immutableClone(artifact) as Artifact; const bucket = this.#artifacts.get(scopeKey(artifact.scope)) ?? new Map<string, Artifact>(); if (bucket.has(artifact.id)) throw new Error(`Artifact "${artifact.id}" is immutable and cannot be overwritten`); bucket.set(artifact.id, frozen); this.#artifacts.set(scopeKey(artifact.scope), bucket); return frozen; }
