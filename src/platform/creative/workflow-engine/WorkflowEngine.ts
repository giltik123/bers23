...
export class CreativeWorkflowEngine {
  ...
  async #runStep(workflow: CompiledWorkflow, operation: WorkflowOperation, router: ArtifactRouter, prior: StepResult[], verification: VerificationResult[], metrics: MutableMetrics, emit: (type: string, stepId?: string, details?: Record<string, unknown>) => void): Promise<StepResult> {
    ...
    const retryBudget = operation.type === 'CONTROLLED_LOCAL_EDIT' ? 0 : workflow.budget.retries;
    while (attempts <= retryBudget) { attempts += 1; emit(attempts > 1 ? 'RECOVERING' : 'READY', operation.id, { providerId });
      ...
      try { emit('RUNNING', operation.id, { providerId }); const controlled = operation.type === 'CONTROLLED_LOCAL_EDIT';
        ...
        const candidates = bound ? bound.map((item) => router.put(item)) : (response.artifacts ?? []).filter(item => item.metadata?.artifactRole !== 'COMPOSITE').map((item) => router.put({ ...item, producerStepId: operation.id, scope: workflow.scope })); emit('VERIFYING', operation.id); const controlledVerification = response.verification; const checked = controlledVerification ?? (this.dependencies.verifier ? await this.dependencies.verifier.verify(operation, candidates) : defaultVerify(operation, candidates)); verification.push(immutableClone(checked) as VerificationResult); if (!checked.valid) throw new Error(checked.errors.join('; ') || 'Verification failed');
        const finals = bound ? [] : (response.artifacts ?? []).filter(item => item.metadata?.artifactRole === 'COMPOSITE').map((item) => router.put({ ...item, producerStepId: operation.id, scope: workflow.scope })); const artifacts = [...candidates, ...finals]; emit('SUCCESS', operation.id); emit('FINISHED', operation.id); return immutableClone({ stepId: operation.id, state: 'FINISHED' as const, attempts, providerId, artifacts });
      } catch (error) { if (isUnknownProviderOutcome(error)) throw error; lastError = error instanceof Error ? error.message : String(error); metrics.failures += 1; emit('FAILED', operation.id, { error: lastError }); if (attempts <= retryBudget) { metrics.retries += 1; providerId = (providerId && this.dependencies.providers.fallback(providerId, operation, workflow.scope)) || providerId; } }
    }
...
function preserveModeOf(value: unknown, fallback: number): number { return typeof value === 'number' && Number.isFinite(value) ? value : fallback; }
