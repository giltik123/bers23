...
export interface CreativePlannerConfigSnapshot { readonly minimumIntentConfidence: number; readonly minimumTargetConfidence: number; readonly maximumPreservationRisk: number; readonly compositeExecutionEnabled: boolean; readonly localCompositeContinuationEnabled: boolean }
...
export type VerificationCheckType = 'OUTPUT_KIND' | 'ARTIFACT_ROLE' | 'PRESERVATION' | 'INTEGRITY' | 'QUALITY_THRESHOLD';
export interface CreativeVerificationSpec { readonly id: string; readonly version: string; readonly checkType: VerificationCheckType; readonly criterion: string; readonly required: boolean; readonly expectedOutputKind: string; readonly expectedArtifactRole: CreativeArtifactRole; readonly constraints: readonly string[]; readonly predicate?: Readonly<{ kind: 'MINIMUM'; threshold: number }>; readonly reasonCode: string }
export type FallbackTrigger = 'VERIFICATION_FAILURE' | 'PROVIDER_UNAVAILABLE' | 'CAPABILITY_MISMATCH' | 'QUALITY_BELOW_THRESHOLD' | 'TIMEOUT';
export type FallbackAction = 'RETRY_SAME' | 'ALTERNATE_CANDIDATE' | 'PARTIAL_REPLAN' | 'ASK_USER' | 'ABORT';
export interface CreativeFallbackAdvice { readonly id: string; readonly version: string; readonly trigger: FallbackTrigger; readonly action: FallbackAction; readonly candidateId: string; readonly stepId: string; readonly alternateCandidateId?: string; readonly maxAttempts: number; readonly maxAdditionalCredits: number; readonly maxAdditionalLatencyMs: number; readonly maxGenerationDepth: number; readonly reasonCode: string; readonly inheritedConstraints: CreativePlanConstraints }
export interface CreativePlanCandidate { readonly id: string; readonly operations: readonly CreativeOperation[]; readonly targetPreference: Exclude<ExecutionTarget, 'BLOCKED'>; readonly estimatedCredits: number; readonly estimatedLatencyMs: number; readonly score: CreativePlanScore; readonly status: 'ACCEPTED' | 'REJECTED'; readonly reasonCodes: readonly string[]; readonly fallbackAdvice?: readonly CreativeFallbackAdvice[] }
...
export interface CreativePlanReplayMetadata { readonly schemaVersion: string; readonly plannerVersion: string; readonly plannerConfig: CreativePlannerConfigSnapshot; readonly operationRulesVersion: string; readonly scoringPolicyVersion: string; readonly verificationPolicyVersion: string; readonly fallbackPolicyVersion: string; readonly inputArtifacts: readonly CreativePlanArtifactSnapshot[]; readonly selectedCandidateId?: string; readonly rejectedCandidates: readonly Readonly<{ id: string; reasonCodes: readonly string[] }>[] }
export interface CreativePlanExplanation { readonly selectedCandidate?: Readonly<{ id: string; reasonCodes: readonly string[] }>; readonly rejectedCandidates: readonly Readonly<{ id: string; reasonCodes: readonly string[] }>[]; readonly materialConstraints: readonly string[]; readonly uncertaintyReasons: readonly string[]; readonly requiredVerificationIds: readonly string[]; readonly fallbackPaths: readonly Readonly<{ id: string; action: FallbackAction; reasonCode: string }>[]; readonly plannerConfig: CreativePlannerConfigSnapshot; readonly versions: Readonly<{ planner: string; schema: string; verificationPolicy: string; fallbackPolicy: string }> }
export interface CreativePlanProvenance { readonly plannerVersion: string; readonly plannerConfig?: CreativePlannerConfigSnapshot; readonly decisionGoal: string; readonly inputArtifacts: readonly CreativePlanArtifactSnapshot[]; readonly reasons: readonly string[]; readonly constraints?: CreativePlanConstraints; readonly chosenCandidateId?: string; readonly rejectedCandidates?: readonly Readonly<{ id: string; reasonCodes: readonly string[] }>[]; readonly scoringRationale?: readonly string[]; readonly replay?: CreativePlanReplayMetadata }
export interface CreativePlan {
  ...
  readonly explanation?: CreativePlanExplanation;
}
...
export type CreativeOperation = WorkflowOperation & Readonly<{ outputArtifacts?: readonly string[]; verification?: readonly CreativeVerificationSpec[] }>;
