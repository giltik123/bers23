...
export function buildReplay(plannerVersion: string, plannerConfig: CreativePlannerConfigSnapshot, plan: Pick<CreativePlan, 'provenance' | 'selectedCandidateId'>): CreativePlanReplayMetadata {
  const inputArtifacts = (plan.provenance?.inputArtifacts ?? []).map(({ id, kind, role }) => ({ id, kind, role }));
  const rejectedCandidates = (plan.provenance?.rejectedCandidates ?? []).map(({ id, reasonCodes }) => ({ id, reasonCodes: [...reasonCodes] }));
  return immutable({ schemaVersion: PLAN_SCHEMA_VERSION, plannerVersion, plannerConfig: { ...plannerConfig }, operationRulesVersion: OPERATION_RULES_VERSION, scoringPolicyVersion: SCORING_POLICY_VERSION, verificationPolicyVersion: VERIFICATION_POLICY_VERSION, fallbackPolicyVersion: FALLBACK_POLICY_VERSION, inputArtifacts, selectedCandidateId: plan.selectedCandidateId, rejectedCandidates });
}
