...
export const CANONICAL_PLANNER_VERSION = '6.40B.2';
type Options = Readonly<{ plannerVersion?: string; minimumIntentConfidence?: number; minimumTargetConfidence?: number; maximumPreservationRisk?: number; compositeExecutionEnabled?: boolean }>;
...
export class CanonicalPlanningService implements CanonicalPlanningPort {
  ...
  async plan(request: CreativeRequest, decision: CreativeDecision): Promise<CreativePlan> {
    ...
    const strategies = composite ? [compositeOperations('local-efficient', artifacts), compositeOperations('cloud-quality', artifacts)] : [simpleOperations(request, artifacts)];
...
function compositeOperations(prefix: string, artifacts: readonly CreativePlanArtifactSnapshot[]): readonly CreativeOperation[] {
  const seed = artifacts.map(a => a.id); const operation = (id: string, type: string, dependencies: readonly string[], requiredArtifacts: readonly string[], output: string): CreativeOperation => ({ id: `${prefix}-${id}`, type, dependencies, requiredArtifacts, produces: ['image'], outputArtifacts: [output] });
