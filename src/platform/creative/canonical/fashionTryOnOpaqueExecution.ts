...
export const FASHION_TRYON_EXECUTION_GRANT_VERSION = '1';
...
const GRANT_KEYS = Object.freeze(['expiresAt','mimeType','outputHeight','outputWidth','phase','ticketId','toolId','toolVersion','version']);
...
export type FashionTryOnExecutionGrantV1 = Readonly<{
  version: typeof FASHION_TRYON_EXECUTION_GRANT_VERSION;
  ticketId: string;
...
export function normalizeFashionTryOnExecutionGrant(value: unknown): FashionTryOnExecutionGrantV1 {
  const record = exactRecord(value, GRANT_KEYS, 'Fashion Try-On execution grant');
  if (record.version !== FASHION_TRYON_EXECUTION_GRANT_VERSION) throw new Error('Fashion Try-On execution grant version is invalid');
  ...
  if (record.toolId !== expected.toolId || record.toolVersion !== expected.toolVersion) throw new Error('Fashion Try-On execution grant tool binding is invalid');
  ...
  requirePixelBudget(outputWidth, outputHeight, expected.maxPixels, 'Fashion Try-On execution grant');
  if (record.mimeType !== FASHION_TRYON_EXECUTION_MIME) throw new Error('Fashion Try-On execution grant mimeType is invalid');
  if (!Number.isSafeInteger(record.expiresAt) || Number(record.expiresAt) < 1) throw new Error('Fashion Try-On execution grant expiresAt is invalid');
...
export function requireUsableFashionTryOnExecutionGrant(
  value: unknown,
  expectedPhase: FashionTryOnExecutionPhase,
  now = Date.now(),
): FashionTryOnExecutionGrantV1 {
  const grant = normalizeFashionTryOnExecutionGrant(value);
  if (grant.phase !== expectedPhase) throw new Error('Fashion Try-On execution grant phase is invalid for this executor');
  if (!Number.isSafeInteger(now) || now < 0) throw new Error('Fashion Try-On execution grant clock is invalid');
  if (grant.expiresAt <= now) throw new Error('Fashion Try-On execution grant is expired');
  return grant;
}
...
export function decodeFashionTryOnTextureExecutionEnvelope(bytes: Uint8Array): FashionTryOnTextureExecutionEnvelopeV1 {
  ...
  return normalizeTextureEnvelope({
    ...record,
    projectRgba: bytes.subarray(payloadOffset, payloadOffset + projectBytes),
    garmentSourceRgba: bytes.subarray(payloadOffset + projectBytes),
...
function normalizeMeshEnvelope(value: FashionTryOnMeshExecutionEnvelopeV1): FashionTryOnMeshExecutionEnvelopeV1 {
  ...
  return deepFreeze({ basisViewRgba, basisViewWidth, basisViewHeight, ...spec });
}
...
function requireUuid(value: unknown, label: string): string {
  if (typeof value !== 'string' || !UUID.test(value)) throw new Error(`Fashion Try-On execution grant ${label} must be a canonical lowercase UUID`);
  return value;
}
function requirePhase(value: unknown): FashionTryOnExecutionPhase {
  if (value !== FASHION_TRYON_MESH_PHASE && value !== FASHION_TRYON_TEXTURE_PHASE) throw new Error('Fashion Try-On execution grant phase is invalid');
  return value;
}
function requireDimension(value: unknown, max: number, label: string): number {
  if (!Number.isSafeInteger(value) || Number(value) < 1 || Number(value) > max) throw new Error(`${label} is outside the admitted execution geometry`);
  return Number(value);
}
function requirePixelBudget(width: number, height: number, maxPixels: number, label: string): void {
  const pixels = width * height;
  if (!Number.isSafeInteger(pixels) || pixels > maxPixels) throw new Error(`${label} exceeds the admitted pixel budget`);
}
function normalizeRgba(value: Uint8Array, width: number, height: number, label: string): Uint8Array {
  const expected = width * height * 4;
  if (!(value instanceof Uint8Array) || !Number.isSafeInteger(expected) || value.byteLength !== expected) throw new Error(`${label} RGBA length is invalid`);
