...
export class MobileSamBrowserSegmentation implements InteractiveSegmentationPort {
  ...
  constructor(private readonly sessions: OnnxSessionFactory, private readonly cache: VerifiedModelCache = new IndexedDbVerifiedModelCache(), private readonly fetcher: typeof fetch = fetch) {}
  ...
  async segment(input: Parameters<InteractiveSegmentationPort['segment']>[0]) {
    ...
    const image = await decodeImage(input.imageArtifactId, this.fetcher); this.assertCurrent(input.requestId);
    ...
    input.points.forEach((point, index) => { coords[index * 2] = (point.x * input.analysis.scaleX + input.analysis.offsetX) * prep.promptScale; coords[index * 2 + 1] = (point.y * input.analysis.scaleY + input.analysis.offsetY) * prep.promptScale; labels[index] = point.label === 'POSITIVE' ? 1 : 0; });
...
async function decodeImage(url: string, fetcher: typeof fetch) { const response = await fetcher(url); if (!response.ok) throw new Error('Selection image is unavailable'); return createImageBitmap(await response.blob()); }
function preprocess(image: ImageBitmap, analysis: AnalysisTransform) {
  ...
  context.drawImage(image, 0, 0, analysis.analysisWidth, analysis.analysisHeight); image.close(); const longest = Math.max(analysis.analysisWidth, analysis.analysisHeight), scale = CANVAS / longest, rw = Math.round(analysis.analysisWidth * scale), rh = Math.round(analysis.analysisHeight * scale);
  const resized = new OffscreenCanvas(CANVAS, CANVAS), target = resized.getContext('2d', { willReadFrequently: true }); if (!target) throw new Error('Canvas unavailable'); target.drawImage(canvas, 0, 0, rw, rh); const rgba = target.getImageData(0, 0, CANVAS, CANVAS).data, data = new Float32Array(3 * CANVAS * CANVAS), plane = CANVAS * CANVAS;
  for (let i = 0; i < plane; i++) for (let channel = 0; channel < 3; channel++) data[channel * plane + i] = (rgba[i * 4 + channel] - PIXEL_MEAN[channel]) / PIXEL_STD[channel];
  return { tensor: tensor(data, [1, 3, CANVAS, CANVAS]), promptScale: scale };
}
