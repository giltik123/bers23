...
function tier(signal: DeviceSignals): DeviceTier {
  ...
  const accelerator = [signal.webgpu, signal.webnn, signal.cuda, signal.directml, signal.metal, signal.vulkan].some((value) => value === true);
  observations++; score += accelerator ? 2 : 0;
  if (signal.storageFreeBytes !== undefined) { observations++; score += signal.storageFreeBytes >= 100e9 ? 2 : signal.storageFreeBytes >= 20e9 ? 1 : 0; }
  const average = observations ? score / observations : 0;
