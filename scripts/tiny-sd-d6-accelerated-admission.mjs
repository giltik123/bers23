...
function canonicalJsonValue(value) {
  ...
  if (Array.isArray(value)) {
    const dense = new Array(value.length);
    for (let index = 0; index < value.length; index += 1) {
      if (!Object.prototype.hasOwnProperty.call(value, index)) {
        throw new TypeError(`D6 evidence contains sparse array element at ${index}`);
      }
      const member = value[index];
      if (member === undefined) throw new TypeError(`D6 evidence contains undefined at array index ${index}`);
      dense[index] = canonicalJsonValue(member);
    }
    return dense;
  }
  if (isRecord(value)) {
