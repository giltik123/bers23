...
upstream architecture in two independent Python child processes with a fixed hash seed, requires
cross-process byte reproducibility, validates ONNX structure, proves PyTorch/ORT parity, and—once
the manifest export identity is PINNED—requires the generated bytes to match that exact size/SHA.
It does not sign, publish or promote.
"""
