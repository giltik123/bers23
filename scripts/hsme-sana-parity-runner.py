...
def runtime_lock_digest(path: Path) -> tuple[dict[str, Any], str]:
    ...
    required_core = {"torch", "diffusers", "transformers", "huggingface-hub", "safetensors"}
    if set(core_artifacts) != required_core:
        raise RunnerError("runtime lock coreArtifacts set mismatch")
