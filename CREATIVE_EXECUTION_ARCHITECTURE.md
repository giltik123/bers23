...
`provider-platform` owns the transport-neutral provider, registry, transport, and artifact-loader contracts. `provider-runtime` is the sole external-I/O boundary and implements HTTP transport, retry/circuit behavior, and integrity-aware artifact downloading. Concrete adapters such as Fal depend only on the provider-platform contracts and receive both transport and artifact loading through dependency injection.

`creative/composition/CreativeProviderComposition.ts` is the only allowed composition root for concrete provider imports. It creates the transport and artifact loader, constructs the Fal adapter, and registers it in `ProviderAdapterRegistry`. Runtime, workflow, pipeline, decision, and execution modules must never import Fal or another concrete adapter. Provider artifacts therefore travel from provider result through the runtime artifact loader and integrity validation before conversion to the canonical artifact contract.

## Sprint 6.29.1 regression matrix

| Suite | Result | Root cause / disposition |
|---|---|---|
| creative-providers | PASS | Provider implementations contain no network calls; the previous artifact-loader `options.fetch` bypass was removed. |
| creative-provider-runtime | PASS | The direct Fal re-export was deleted; runtime imports only provider-platform and the public provider model. |
| creative-provider-intelligence | PASS | No boundary regression. |
| creative-workflow-integration | PASS | No boundary regression. |
| creative-canonical | PASS | Fitness tests enforce concrete-provider composition and forbidden import direction. |
| creative-operations / creative-pipeline | PASS | Neither layer imports a concrete provider. |
| creative-production / creative-hybrid | PASS | Existing infrastructure and compatibility behavior remains intact. |
| creative-local-ai / creative-decision-model | PASS | Local and decision isolation remains intact. |
| all `creative-*.test.ts` suites | PASS | 43 suites; no pre-existing or introduced failures remain. |

**PRE-EXISTING FAILURE:** none. **INTRODUCED FAILURE:** none. The two failures inherited from Sprint 6.29 were closed at their root causes rather than suppressed.
