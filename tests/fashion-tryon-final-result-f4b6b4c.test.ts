...
test('F4b.6b.4c derives exact texture phase and returns only signed FINAL candidate for the exact READY intent', async () => {
  const h = harness();
...
test('F4b.6b.4c maps durable texture states to closed orchestration states without internal execution identity', async () => {
  const cases = [
