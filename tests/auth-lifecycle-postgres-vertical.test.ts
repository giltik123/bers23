...
async function post(baseUrl:string,path:string,body:unknown,token?:string){return fetch(`${baseUrl}${path}`,{method:'POST',headers:{'content-type':'application/json',...(token?{authorization:`Bearer ${token}`}:{})},body:JSON.stringify(body)});}
...
function fragmentValue(location:string,key:string){const url=new URL(location);assert.equal(url.searchParams.has(key),false);return new URLSearchParams(url.hash.slice(1)).get(key);}
...
async function createProject(running:Running,token:string,name:string){const png=await sharp({create:{width:3,height:2,channels:4,background:{r:12,g:34,b:56,alpha:1}}}).png().toBuffer();return fetch(`${running.baseUrl}/api/core/projects?name=${encodeURIComponent(name)}`,{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'image/png'},body:png});}
async function register(running:Running,email:string,password:string){const response=await post(running.baseUrl,'/api/core/auth/register',{email,password});assert.equal(response.status,202);const body=await response.json() as any;assert.equal(body.status,'verification_required');assert.match(String(body.verification_handle),/^[A-Za-z0-9_-]{40,128}$/);return body;}
async function callback(running:Running,flow:{state:string;nonce:string},code:string,idToken:string){oauthTokens.set(code,idToken);return fetch(`${running.baseUrl}/api/core/auth/callback/google?state=${encodeURIComponent(flow.state)}&code=${encodeURIComponent(code)}`,{redirect:'manual'});}
...
test('registration, recovery and Google OAuth remain fail-closed against account takeover and secret leakage',async t=>{
  ...
  // A racing registration inside cooldown has the same public shape but cannot alter credential or mail state.
  ...
  // After cooldown a competing attempt may replace pending data, but only its own secret handle can authorize that attempt.
  ...
  // The legitimate owner can start a fresh attempt; wrong-handle resend is a generic no-op.
  ...
  const verified=await post(running.baseUrl,'/api/core/auth/verify-otp',{email,otpCode:ownerResentCode,verificationHandle:ownerAttempt2.verification_handle});assert.equal(verified.status,200);const verifiedBody=await verified.json() as any;const localToken=String(verifiedBody.access_token);assert.equal(verifiedBody.user.email_verified,true);assert.equal(verifiedBody.user.id,pending.user_id);
  assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:attackerPassword})).status,401);assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:ownerPassword})).status,200);
  assert.equal((await createProject(running,localToken,'Local Auth Project')).status,201);

  // Active-account registration is enumeration-resistant and cannot replace its credential.
  const activeMailCount=mails.length;const activeRegistration=await register(running,email,attackerPassword);assert.equal(mails.length,activeMailCount);assert.ok(activeRegistration.verification_handle);assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:ownerPassword})).status,200);assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:attackerPassword})).status,401);

  // Expired verification cannot create a session.
  ...
  // Password reset is enumeration-resistant, fragment-only, single-use, and revokes all existing sessions.
  ...
  const passwordReset=await post(running.baseUrl,'/api/core/auth/password/reset',{resetToken:reset,newPassword});assert.equal(passwordReset.status,200);assert.equal((await fetch(`${running.baseUrl}/api/core/auth/context`,{headers:{authorization:`Bearer ${localToken}`}})).status,401);
  assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:ownerPassword})).status,401);const newLogin=await post(running.baseUrl,'/api/core/auth/password/login',{email,password:newPassword});assert.equal(newLogin.status,200);const newLocalToken=String((await newLogin.json() as any).access_token);
  assert.equal((await post(running.baseUrl,'/api/core/auth/password/reset',{resetToken:reset,newPassword:'another secure password'})).status,400);assert.equal((await fetch(`${running.baseUrl}/api/core/projects`,{headers:{authorization:`Bearer ${newLocalToken}`}})).status,200);

  // Invalid OAuth state fails before token exchange.
  ...
  async function rejectedGoogle(code:string,tokenFactory:(nonce:string)=>string){const flow=await startGoogle(running);const response=await callback(running,flow,code,tokenFactory(flow.nonce));assert.equal(response.status,401);}
  ...
  // return_to is same-origin only, including backslash/network-path tricks.
  const redirectFlow=await startGoogle(running,'/\\evil.example/steal');const redirectResponse=await callback(running,redirectFlow,'safe-redirect',signGoogle(claims(redirectFlow.nonce,{sub:'redirect-user',email:'redirectuser@gmail.com'})));assert.equal(redirectResponse.status,302);const safeLocation=new URL(String(redirectResponse.headers.get('location')));assert.equal(safeLocation.origin,config.authPublicOrigin);assert.equal(safeLocation.pathname,'/');assert.equal(safeLocation.searchParams.has('auth_code'),false);assert.ok(fragmentValue(safeLocation.toString(),'auth_code'));

  // Verified Gmail can link to the existing verified local account; grant is fragment-only and single-use.
  const linkFlow=await startGoogle(running,'/projects');const linkCallback=await callback(running,linkFlow,'link-local',signGoogle(claims(linkFlow.nonce,{sub:'linked-google-sub',email})));assert.equal(linkCallback.status,302);const linkLocation=new URL(String(linkCallback.headers.get('location')));assert.equal(linkLocation.origin,config.authPublicOrigin);assert.equal(linkLocation.pathname,'/projects');assert.equal(linkLocation.searchParams.has('access_token'),false);const linkGrant=fragmentValue(linkLocation.toString(),'auth_code');assert(linkGrant);
  const linkExchange=await post(running.baseUrl,'/api/core/auth/exchange',{code:linkGrant});assert.equal(linkExchange.status,200);const linkBody=await linkExchange.json() as any;assert.equal(linkBody.user.id,pending.user_id);assert.equal((await post(running.baseUrl,'/api/core/auth/exchange',{code:linkGrant})).status,401);

  // Google proof must never activate an unverified local password set before ownership was proven.
  ...
  const pendingGoogleFlow=await startGoogle(running);const pendingGoogleCallback=await callback(running,pendingGoogleFlow,'pending-google',signGoogle(claims(pendingGoogleFlow.nonce,{sub:'pending-google-sub',email:pendingGoogleEmail})));assert.equal(pendingGoogleCallback.status,302);const pendingGoogleGrant=fragmentValue(String(pendingGoogleCallback.headers.get('location')),'auth_code');assert(pendingGoogleGrant);assert.equal((await post(running.baseUrl,'/api/core/auth/exchange',{code:pendingGoogleGrant})).status,200);assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email:pendingGoogleEmail,password:pendingGooglePassword})).status,401);assert.equal(Number((await pool.query('SELECT count(*)::int AS count FROM canonical_auth_password_credentials WHERE user_id=$1',[pendingGoogleUser.user_id])).rows[0].count),0);

  // Existing non-authoritative corporate email cannot be auto-linked by email alone.
  ...
  // New non-Gmail Google identity is keyed by sub but cannot use email password recovery until canonical email verification exists.
  const externalFlow=await startGoogle(running);const externalCallback=await callback(running,externalFlow,'external-google',signGoogle(claims(externalFlow.nonce,{sub:'external-google-sub',email:'external@example.org',hd:undefined})));assert.equal(externalCallback.status,302);const externalGrant=fragmentValue(String(externalCallback.headers.get('location')),'auth_code');assert(externalGrant);const externalExchange=await post(running.baseUrl,'/api/core/auth/exchange',{code:externalGrant});assert.equal(externalExchange.status,200);const externalBody=await externalExchange.json() as any;assert.equal(externalBody.user.email_verified,false);const mailsBeforeExternalReset=mails.length;assert.equal((await post(running.baseUrl,'/api/core/auth/password/reset-request',{email:'external@example.org'})).status,202);assert.equal(mails.length,mailsBeforeExternalReset);

  const googleFlow=await startGoogle(running,'/projects');const googleCallback=await callback(running,googleFlow,'good-google',signGoogle(claims(googleFlow.nonce,{sub:'new-google-sub',email:'newoauth@gmail.com'})));assert.equal(googleCallback.status,302);const googleLocation=new URL(String(googleCallback.headers.get('location')));assert.equal(googleLocation.searchParams.has('auth_code'),false);const googleGrant=fragmentValue(googleLocation.toString(),'auth_code');assert(googleGrant);const googleExchange=await post(running.baseUrl,'/api/core/auth/exchange',{code:googleGrant});assert.equal(googleExchange.status,200);const googleBody=await googleExchange.json() as any;const googleToken=String(googleBody.access_token);assert.equal(googleBody.user.email,'newoauth@gmail.com');assert.equal((await createProject(running,googleToken,'Google Auth Project')).status,201);assert.equal((await fetch(`${running.baseUrl}/api/core/projects`,{headers:{authorization:`Bearer ${googleToken}`}})).status,200);assert.equal((await pool.query("SELECT count(*)::int AS count FROM canonical_auth_oauth_identities WHERE provider='google' AND provider_subject='new-google-sub'")).rows[0].count,1);

  await stop(running);running=await start();assert.equal((await fetch(`${running.baseUrl}/api/core/auth/context`,{headers:{authorization:`Bearer ${googleToken}`}})).status,200);assert.equal((await post(running.baseUrl,'/api/core/auth/exchange',{code:googleGrant})).status,401);assert.equal((await post(running.baseUrl,'/api/core/auth/password/reset',{resetToken:reset,newPassword:'yet another secure password'})).status,400);
