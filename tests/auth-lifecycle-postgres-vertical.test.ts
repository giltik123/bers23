...
test('registration, recovery and Google OAuth remain fail-closed against account takeover and secret leakage',async t=>{
  ...
  const passwordReset=await post(running.baseUrl,'/api/core/auth/password/reset',{resetToken:reset,newPassword},localCookie);assert.equal(passwordReset.status,200);assert.match(String(passwordReset.headers.get('set-cookie')),/^bers_session_dev=;/);assert.equal((await fetch(`${running.baseUrl}/api/core/auth/context`,{headers:browser(localCookie)})).status,401);
  assert.equal((await post(running.baseUrl,'/api/core/auth/password/login',{email,password:ownerPassword})).status,401);const newLogin=await post(running.baseUrl,'/api/core/auth/password/login',{email,password:newPassword});assert.equal(newLogin.status,200);const newLocalCookie=sessionCookie(newLogin);const newLoginBody=await newLogin.json() as any;assert.equal('access_token' in newLoginBody,false);
  assert.equal((await post(running.baseUrl,'/api/core/auth/password/reset',{resetToken:reset,newPassword:'another secure password'})).status,400);assert.equal((await fetch(`${running.baseUrl}/api/core/projects`,{headers:browser(newLocalCookie)})).status,200);

  const noStateTokenCalls=tokenExchanges;const badState=await fetch(`${running.baseUrl}/api/core/auth/callback/google?state=invalid-state&code=never`,{redirect:'manual',headers:browser()});assert.equal(badState.status,401);assert.equal(tokenExchanges,noStateTokenCalls);
  ...
  const linkExchange=await post(running.baseUrl,'/api/core/auth/exchange',{code:linkGrant});assert.equal(linkExchange.status,200);const linkCookie=sessionCookie(linkExchange);const linkBody=await linkExchange.json() as any;assert.equal('access_token' in linkBody,false);assert.equal(linkBody.user.id,pending.user_id);assert.equal((await fetch(`${running.baseUrl}/api/core/auth/context`,{headers:browser(linkCookie)})).status,200);assert.equal((await post(running.baseUrl,'/api/core/auth/exchange',{code:linkGrant})).status,401);
