...
import { MAX_LICENSE_EVIDENCE_BYTES, assertPinnedLicenseEvidence } from '../scripts/kandinsky-license-trust.mjs';
...
function assertLicenseEvidence(evidence) {
...
test('F5b.1 pins exact safe source weights, config bytes and revision license metadata but grants no runtime authority', () => {
  assert.equal(manifest.modelId, 'kandinsky-2-2-decoder-inpaint-refinement');
  assert.equal(manifest.version, '0.1.0-feasibility.3');
  assert.equal(manifest.status, 'CANDIDATE');
  assert.equal(manifest.artifactState, 'SOURCE_WEIGHT_CONFIG_AND_LICENSE_TRUST_PINNED_CONDITIONING_AND_RUNTIME_UNPROVEN');
  ...
  assert.equal(manifest.licenseReview.state, 'PINNED_UPSTREAM_METADATA_PRODUCT_REVIEW_REQUIRED');
...
test('F5b.1 decoder source/config/license trust roots are exact and revision-bound', () => {
  ...
  assert.equal(manifest.decoder.license, 'apache-2.0');
  assertLicenseEvidence(manifest.decoder.licenseEvidence);
  ...
  assert.deepEqual(manifest.decoder.safeWeights, [
    { path: 'unet/diffusion_pytorch_model.safetensors', size: 5_012_378_704, sha256: '098b846d2378b4b44a33e0bd47f89f2886b72a10200cbf9e96d5fae3c471543f' },
    { path: 'movq/diffusion_pytorch_model.safetensors', size: 271_380_364, sha256: '43a5860fea195a7116f2471396c5cc9535fade9b63c4857d8a192ffd924b7002' },
  ]);
...
test('F5b.1 prior is offline-only exact source/config/license trust evidence', () => {
  ...
  assert.equal(manifest.offlinePrior.license, 'apache-2.0');
  assertLicenseEvidence(manifest.offlinePrior.licenseEvidence);
