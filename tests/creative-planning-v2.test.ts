...
const testProviderSelection = { select: () => ({ allowed: true as const, reasonCode: 'PROVIDER_SELECTED' as const, providerId: 'synthetic-planning-provider', selectionId: 'synthetic-planning-provider:v1' }) };
...
test('forged target/cost cannot bypass target/security or cause billing, and non-ready plans have zero side effects', async () => {
  for (const status of ['NEEDS_CONFIRMATION', 'BLOCKED'] as const) {
    ...
    const platform = new CreativeExecutionPlatform({ decision: { decide: async r => ({ requestId: r.id, goal: r.intent, constraints: [] }) }, planning: { plan: async r => ({ requestId: r.id, status, operations: [{ id: 'forged', type: 'x', providerId: 'cloud', cost: { credits: 999 } }] }) }, targetSelector: { select: () => { calls.target++; return 'CLOUD'; } }, providerSelector: { select: () => { calls.provider++; return testProviderSelection.select(); } }, capabilityAdmission: testCapabilityAdmission, securityGate: { authorize: () => { calls.security++; return false; } }, runtime: { execute: async () => { calls.runtime++; return {}; } }, providers: { isAvailable: () => true, fallback: () => undefined }, recovery: { decide: () => 'ABORT' }, billing: { reserve: async () => { calls.reserve++; }, commit: async () => {}, release: async () => {} } });
    platform.createExecution({ id: `forged-${status}`, intent: 'x', scope }); await assert.rejects(platform.execute(`forged-${status}`), /not executable/); assert.deepEqual(calls, { target: 0, provider: 0, security: 0, runtime: 0, reserve: 0 });
  }
});
