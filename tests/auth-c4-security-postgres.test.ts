    ...
    try {
      ...
      await t.test('HTTP boundary ignores spoofed proxy headers and protects revoke-all with CSRF/origin', async () => {
        ...
        const adapter = createNodeHttpAdapter({
          core: {} as any, artifacts: {} as any, projects: {} as any, auth: harness.service, config: config as any,
...
function makeService(pool: Pool, clock: ReturnType<typeof mutableClock>, options: any = {}) {
  ...
  const google = options.google ?? {
    authorizationUrl: ({ state, nonce }: { state: string; nonce: string }) => `https://oauth.example.test/authorize?state=${encodeURIComponent(state)}&nonce=${encodeURIComponent(nonce)}`,
...
function captureMail() {
  const state: any = {
    ...
    resetUrl: undefined,
    sender: {
      sendVerification: async ({ code }: { code: string }) => { state.verificationCode = code; },
      sendPasswordReset: async ({ resetUrl }: { resetUrl: string }) => { state.resetUrl = resetUrl; },
    },
  };
  return state;
}
...
function jwtSessionId(token: string) { return JSON.parse(Buffer.from(token.split('.')[1], 'base64url').toString('utf8')).sid; }
function hasCode(code: string) { return (error: any) => Boolean(error && error.code === code); }
