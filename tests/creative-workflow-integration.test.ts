...
test('integration layer has no provider-specific imports', () => { const directory = 'src/platform/creative/integration'; const forbidden = ['providers/', 'workflow/', 'runtime/', 'billing/', 'application/', 'ui/', 'infrastructure/', 'react', 'retired-runtime']; for (const file of readdirSync(directory)) { if (!file.endsWith('.ts')) continue; const imports = readFileSync(`${directory}/${file}`, 'utf8').split('\n').filter((line) => /^import|^export .* from/.test(line)).join('\n').toLowerCase(); for (const term of forbidden) assert.equal(imports.includes(term), false, `${file} imports ${term}`); } });
