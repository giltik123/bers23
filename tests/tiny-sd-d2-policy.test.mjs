...
test('D2 hosted workflow destroys binary/model/reference evidence before JSON upload', () => {
  ...
  const tail = workflow.slice(destroy, upload);
  assert.match(tail, /rm -rf/);
  assert.match(tail, /tiny-sd-d2-reference\.npz/);
  assert.match(tail, /tiny-sd-d2-onnx/);
  assert.doesNotMatch(workflow.slice(upload), /\.onnx/);
  assert.doesNotMatch(workflow.slice(upload), /\.safetensors/);
  assert.doesNotMatch(workflow.slice(upload), /\.npz/);
});
