...
test('D3 full matrix binds its computed winner to one checked-in accepted selection policy', () => {
  assert.match(matrix, /ACCEPTED_SELECTED_STRATEGY_BY_COMPONENT = \{/);
  assert.match(matrix, /ACCEPTED_SELECTED_SCHEME_BY_COMPONENT = \{/);
  ...
  assert.match(matrix, /def accepted_strategy_definition\(/);
  assert.match(matrix, /min\(passing, key=lambda item: \(item\[0\], item\[1\]\)\)/);
  assert.match(matrix, /selected_name != expected_selected_name/);
  assert.match(matrix, /selected_scheme != expected_selected_scheme/);
  assert.match(matrix, /"acceptedSelectionPolicyMatched": accepted_selection_policy_matched/);
  assert.match(matrix, /"selectionPolicyMismatches": policy_mismatches/);
  assert.match(matrix, /"selectionPolicyUpdateRequiredOnWinnerChange": True/);
  assert.match(matrix, /if not accepted_selection_policy_matched:/);
  assert.match(matrix, /explicit accepted-selection policy update required/);
});
...
test('D4 reproduces the accepted D3 selected representation without rerunning strategy selection', () => {
  ...
  assert.match(selected, /matrix\.accepted_strategy_definition\(component\)/);
  assert.match(selected, /matrix\.ACCEPTED_SELECTED_STRATEGY_BY_COMPONENT/);
  assert.match(selected, /matrix\.ACCEPTED_SELECTED_SCHEME_BY_COMPONENT/);
  ...
  assert.match(selected, /selectionPolicySource/);
  assert.match(selected, /"acceptedSelectionPolicyMatched": True/);
  ...
  assert.doesNotMatch(selected, /SELECTED_SCHEME\s*=|SELECTED_STRATEGY\s*=/);
  assert.doesNotMatch(selected, /matrix\.exact_fp16_storage_fp32_compute/);
