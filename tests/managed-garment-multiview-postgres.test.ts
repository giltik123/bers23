...
function authHeaders(token: 'owner-token' | 'other-token', expectedRevision?: number | string) { return { authorization: `Bearer ${token}`, origin: 'http://client.test', ...(expectedRevision !== undefined ? { 'x-expected-garment-revision': String(expectedRevision) } : {}) }; }
...
test('F1b appends immutable views to one garment with revision concurrency and truthful capture assessment', async t => {
  ...
  const preflight = await fetch(`${origin}/api/core/garments/example/views`, { method: 'OPTIONS', headers: { origin: 'http://client.test', 'access-control-request-method': 'POST', 'access-control-request-headers': 'content-type,x-expected-garment-revision' } });
  assert.equal(preflight.status, 204); assert.match(preflight.headers.get('access-control-allow-headers') ?? '', /X-Expected-Garment-Revision/); assert.match(preflight.headers.get('access-control-expose-headers') ?? '', /X-Garment-Revision/);
  ...
  assert.equal(createdResponse.status, 201); assert.equal(createdResponse.headers.get('x-garment-revision'), '1'); assert.equal(createdResponse.headers.get('etag'), null, 'garment JSON must not claim a strong ETag while delivery capabilities are time-varying'); const created = await createdResponse.json() as any;
  ...
  const backResponse = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=BACK`, { method: 'POST', headers: { ...authHeaders('owner-token', 1), 'content-type': 'image/jpeg' }, body: await jpeg(200, 700, 4) });
  assert.equal(backResponse.status, 201); assert.equal(backResponse.headers.get('x-garment-revision'), '2'); const afterBack = await backResponse.json() as any;
  ...
  const stale = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=LEFT`, { method: 'POST', headers: { ...authHeaders('owner-token', 1), 'content-type': 'image/jpeg' }, body: await jpeg(620, 620, 5) });
  ...
  const stolen = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=LEFT`, { method: 'POST', headers: { ...authHeaders('other-token', 2), 'content-type': 'image/jpeg' }, body: await jpeg(620, 620, 6) });
  ...
  const [leftAttempt, rightAttempt] = await Promise.all([
    fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=LEFT`, { method: 'POST', headers: { ...authHeaders('owner-token', 2), 'content-type': 'image/jpeg' }, body: leftBytes }),
    fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=RIGHT`, { method: 'POST', headers: { ...authHeaders('owner-token', 2), 'content-type': 'image/jpeg' }, body: rightBytes }),
  ]);
  ...
  const completedResponse = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=${remainingKind}`, { method: 'POST', headers: { ...authHeaders('owner-token', 3), 'content-type': 'image/jpeg' }, body: await jpeg(640, 640, 9) });
  assert.equal(completedResponse.status, 201); assert.equal(completedResponse.headers.get('x-garment-revision'), '4'); const completed = await completedResponse.json() as any;
  ...
  const betterBackResponse = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=BACK`, { method: 'POST', headers: { ...authHeaders('owner-token', 4), 'content-type': 'image/jpeg' }, body: await jpeg(700, 700, 10) });
  assert.equal(betterBackResponse.status, 201); assert.equal(betterBackResponse.headers.get('x-garment-revision'), '5'); const improved = await betterBackResponse.json() as any;
  ...
  const getResponse = await fetch(`${origin}/api/core/garments/${stableGarmentId}`, { headers: authHeaders('owner-token') }); assert.equal(getResponse.status, 200); assert.equal(getResponse.headers.get('x-garment-revision'), '5'); const reloaded = await getResponse.json() as any; assert.equal(reloaded.revision, 5); assert.equal(reloaded.views.length, 5); assert.equal(reloaded.capture_assessment.cardinal_complete, true); assert.equal(reloaded.capture_assessment.technical_resolution.status, 'ADEQUATE');
  const invalidId = await fetch(`${origin}/api/core/garments/not-a-uuid/views?view=DETAIL`, { method: 'POST', headers: { ...authHeaders('owner-token', 5), 'content-type': 'image/jpeg' }, body: await jpeg(600, 600, 11) }); assert.equal(invalidId.status, 404); assert.equal((await invalidId.json() as any).error, 'garment_not_found'); assert.equal((await store.get(owner, stableGarmentId))?.revision, 5);

  await pool.query(`UPDATE canonical_garments SET status='ARCHIVED' WHERE garment_id=$1 AND tenant_id=$2 AND user_id=$3`, [stableGarmentId, owner.tenantId, owner.userId]);
  const archivedAppend = await fetch(`${origin}/api/core/garments/${stableGarmentId}/views?view=DETAIL`, { method: 'POST', headers: { ...authHeaders('owner-token', 5), 'content-type': 'image/jpeg' }, body: await jpeg(600, 600, 12) });
  assert.equal(archivedAppend.status, 409); assert.equal((await archivedAppend.json() as any).error, 'garment_not_active'); const archived = await store.get(owner, stableGarmentId); assert.equal(archived?.revision, 5); assert.equal(archived?.views.length, 5);
});
