...
test('reachable Fashion and Outfits use canonical authorities while later execution verticals remain fail-closed', async () => {
  ...
  assert.match(outfits, /reference readiness are owned by the canonical Outfit authority/);
  ...
  assert.match(creative, /outfits: \[\]/, 'Creative Studio must explicitly remain without Outfit data until its own cutover');
