...
test('F4b.6c.2c HTTP rejects browser storage hash sequence and producer authority before service', async () => {
  await withHttpServer(acquisitionResult, async (base, calls) => {
    for (const extra of [
      { anchorSetId },
...
test('F4b.6c.2c HTTP enforces POST JSON origin OPTIONS and session-bound CSRF', async () => {
