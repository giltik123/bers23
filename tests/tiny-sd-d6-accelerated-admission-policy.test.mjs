...
test('asynchronous attestation verifier can approve the exact canonical payload', async () => {
  const evidence = validDeviceEvidence();
  const verify = verifierFor(evidence);
  const result = await assessTinySdD6RealDeviceEvidence(evidence, {
    expectedTestedCommitSha: TESTED_COMMIT_SHA,
    trustVerifier: async input => Promise.resolve(verify(input)),
    now,
  });
...
test('rejected, thrown and malformed verifier results fail closed', async t => {
  const verifiers = {
    rejected: async () => Promise.reject(new Error('signature service unavailable')),
    thrown: () => { throw new Error('signature verification failed'); },
    malformed: async () => ({ verified: 'true' }),
  };

  for (const [name, trustVerifier] of Object.entries(verifiers)) {
    await t.test(name, async () => {
      const result = await assessTinySdD6RealDeviceEvidence(validDeviceEvidence(), {
        expectedTestedCommitSha: TESTED_COMMIT_SHA,
        trustVerifier,
        now,
      });
...
test('Mesa software renderers cannot claim physical-device admission', async t => {
  for (const renderer of ['llvmpipe', 'lavapipe']) {
    await t.test(renderer, async () => {
      const evidence = validDeviceEvidence();
      evidence.device.architecture = renderer;
      evidence.device.description = `Mesa ${renderer} adapter`;
      const result = await assessTrusted(evidence);
      assert.equal(result.eligible, false);
      assert.ok(result.blockers.includes('SOFTWARE_OR_UNKNOWN_ADAPTER'));
    });
