...
function fixture() {
  ...
  const dependencies: CreativeCoreDependencies = {
    ...
    platform: {
      ...
      targetSelector: { select: () => 'CLOUD' }, providerSelector: { select: () => ({ allowed: true, reasonCode: 'PROVIDER_SELECTED', providerId: 'fal', selectionId: 'synthetic-http:fal' }) }, capabilityAdmission: { admit: () => ({ allowed: true, reasonCode: 'CAPABILITY_SUPPORTED', capabilityId: 'synthetic-http-test-runtime' }) }, securityGate: { authorize: request => request.scope.userId === 'user-a' }, recovery: { decide: () => 'MARK_UNKNOWN' },
...
test('startup validation fails before serving and production server has no browser provider import', async () => { const f = fixture(); assert.throws(() => createCreativeCore({ ...f.dependencies, auth: undefined } as never), /authentication verifier/); assert.throws(() => createCreativeCore({ ...f.dependencies, platform: { ...f.dependencies.platform, providerSelector: undefined } } as never), /provider selection/); assert.throws(() => createCreativeCore({ ...f.dependencies, platform: { ...f.dependencies.platform, capabilityAdmission: undefined } } as never), /execution capability admission/); const source = await import('node:fs/promises').then(fs => fs.readFile('server/core/composition/createCreativeCore.ts', 'utf8')); assert.equal(source.includes('src/lib/editing/'), false); });
