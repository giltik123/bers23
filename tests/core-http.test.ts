...
test('startup validation fails before serving and production server has no browser provider import', async () => { const f = fixture(); assert.throws(() => createCreativeCore({ ...f.dependencies, auth: undefined } as never), /authentication verifier/); const source = await import('node:fs/promises').then(fs => fs.readFile('server/core/composition/createCreativeCore.ts', 'utf8')); assert.equal(source.includes('src/lib/editing/'), false); });
