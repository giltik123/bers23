...
function fixture() {
  ...
  const dependencies: CreativeCoreDependencies = {
    ...
    platform: {
      ...
      planning: { plan: async request => ({ requestId: request.id, operations: [{ id: 'edit', type: 'image-edit', requiredArtifacts: ['artifact-a'], produces: ['result'], providerId: 'forged-http-planner', cost: { credits: 1 } }] }) },
      routeSelector: { select: () => 'PROVIDER' }, targetSelector: { select: () => 'CLOUD' }, providerSelector: { select: () => ({ allowed: true, reasonCode: 'PROVIDER_SELECTED', providerId: 'fal', selectionId: 'synthetic-http:fal' }) }, capabilityAdmission: { admit: () => ({ allowed: true, reasonCode: 'CAPABILITY_SUPPORTED', capabilityId: 'synthetic-http-test-runtime' }) }, securityGate: { authorize: request => request.scope.userId === 'user-a' }, recovery: { decide: () => 'MARK_UNKNOWN' },
      providers: { isAvailable: () => true, fallback: () => undefined },
      runtime: { execute: async request => { providerCalls++; assert.equal(request.operation.providerId, 'fal'); return { artifacts: [{ id: 'result', kind: 'image', value: 'asset://result' }] }; } },
...
test('startup validation fails before serving and production server has no browser provider import', async () => { const f = fixture(); assert.throws(() => createCreativeCore({ ...f.dependencies, auth: undefined } as never), /authentication verifier/); assert.throws(() => createCreativeCore({ ...f.dependencies, platform: { ...f.dependencies.platform, routeSelector: undefined } } as never), /route selection/); assert.throws(() => createCreativeCore({ ...f.dependencies, platform: { ...f.dependencies.platform, providerSelector: undefined } } as never), /provider selection/); assert.throws(() => createCreativeCore({ ...f.dependencies, platform: { ...f.dependencies.platform, capabilityAdmission: undefined } } as never), /execution capability admission/); const source = await import('node:fs/promises').then(fs => fs.readFile('server/core/composition/createCreativeCore.ts', 'utf8')); assert.equal(source.includes('src/lib/editing/'), false); });
