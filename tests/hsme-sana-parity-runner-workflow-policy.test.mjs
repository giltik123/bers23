...
test('metadata pin stays metadata-only and does not install the heavyweight runtime', () => {
  ...
  const end = evidence.indexOf('exact-parity:');
...
test('rights and exact parity are verified before heavyweight runtime installation', () => {
  const planCheck = evidence.indexOf('Verify immutable main candidate and committed reviewed plan before model network work');
...
test('binary model, tensor, cache and image evidence is destroyed before upload', () => {
  const cleanup = evidence.indexOf('Destroy all model/tensor/cache/image bytes before evidence upload');
...
test('runtime lock is exact and quality-first', () => {
  ...
  assert.deepEqual(Object.keys(runtimeLock.coreArtifacts).sort(), [
    'diffusers', 'huggingface-hub', 'safetensors', 'torch', 'transformers',
  ]);
...
test('ordinary contract workflow stays bounded and synthetic', () => {
  ...
  assert.match(evidence, /Fetch and verify exact core runtime artifacts/);
  assert.match(evidence, /Validate candidate JSON through accepted Phase-1A contract/);
