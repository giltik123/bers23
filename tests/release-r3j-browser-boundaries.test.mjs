...
test('cumulative R3 workflow runs R3j after the already accepted R3i journey', async () => {
  ...
  assert.match(workflow, /node --test tests\/release-r3j-browser-boundaries\.test\.mjs/);
  assert.match(workflow, /R3J_BROWSER_CANONICAL_TRYON_ACCEPTED/);
  assert.match(workflow, /response\.clone\(\)\.arrayBuffer\(\)/);
  assert.match(workflow, /diagnostics\.legacyTryOnRequests/);
});
