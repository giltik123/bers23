...
function meshEnvelope(): Uint8Array {
...
test('F4b.6b.4b prepared mesh executor is pixel-identical and submits one identifier-minimal PNG candidate', async () => {
  ...
  }, () => { const value = clock; clock += 7; return value; });
...
test('F4b.6b.4b prepared texture executor is pixel-identical and candidate acknowledgement reveals no FINAL identity', async () => {
  ...
  }, () => { const value = clock; clock += 11; return value; });
...
test('F4b.6b.4b browser delegates absolute descriptor expiry to authenticated Core authority', async () => {
  const envelope = meshEnvelope();
  const calls: any = { loads: 0, submits: 0 };
  const executor = new CorePreparedGarmentMeshWarp(projectId, {
    loadPreparedGarmentMeshWarpInput: async () => { calls.loads += 1; return envelope; },
    submitPreparedGarmentMeshWarpCandidate: async () => { calls.submits += 1; return { status: 'SUCCESS' }; },
  }, () => 1);

  const result = await executor.run(meshDescriptor({ expiresAt: 1 }));
  assert.equal(result.target, 'LOCAL');
  assert.equal(calls.loads, 1, 'Core must decide whether the server-minted ticket is still usable');
  assert.equal(calls.submits, 1);
});

test('F4b.6b.4b prepared executor rejects malformed descriptor or payload geometry before candidate submit', async () => {
  const envelope = meshEnvelope();
  let submits = 0;
  const executor = new CorePreparedGarmentMeshWarp(projectId, {
  ...
  await assert.rejects(
    () => executor.run({ ...meshDescriptor(), phase: FASHION_TRYON_TEXTURE_PHASE }),
    /tool binding is invalid/i,
  );
  await assert.rejects(
    () => executor.run(textureDescriptor()),
    /phase is invalid/i,
  );
