...
test('F4b.4 admission runtime graph stays on narrow GarmentMeshWarp leaves instead of aggregate registries', async () => {
  ...
  assert.equal(graph.includes(aggregateRegistryPath), false, `F4b.4 runtime graph reached ${aggregateRegistryPath}`);
  assert.equal(graph.includes(aggregatePolicyPath), false, `F4b.4 runtime graph reached ${aggregatePolicyPath}`);
