...
export class CreativeExecutionService {
  ...
  execute(command: CreativeEditCommand, auth: AuthenticatedScope): Promise<ProductionOutcome> {
    ...
    const task = this.#execute(command, auth, key); this.#inflight.set(key, task); return task;
  }
  ...
  async #execute(command: CreativeEditCommand, auth: AuthenticatedScope, key: string): Promise<ProductionOutcome> {
