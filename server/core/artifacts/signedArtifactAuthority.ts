...
export class SignedArtifactAuthority {
  ...
  issueStoredOriginal(storageId: string, scope: AuthenticatedScope & { projectId: string }): string { return this.sign({ v: 1, location: 'STORED_ORIGINAL_ID', storageId, ...scope, role: 'ORIGINAL', lifecycle: 'IMMUTABLE' }); }
  issueStoredOriginalDelivery(storageId: string, scope: AuthenticatedScope & { projectId: string }, expiresAt: number): string { if (!Number.isFinite(expiresAt) || expiresAt <= this.#now()) throw denied(); return this.sign({ v: 1, location: 'STORED_ORIGINAL_DELIVERY', storageId, ...scope, role: 'ORIGINAL', lifecycle: 'IMMUTABLE', exp: expiresAt }); }
