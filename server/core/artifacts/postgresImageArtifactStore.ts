...
export type GarmentAppearanceRefinementFinalImageLineage = GarmentAppearanceRefinementFinalLineageV1;
...
export type StoredFinalImage = Readonly<{
  ...
  refinementContractVersion?: GarmentAppearanceRefinementFinalLineageV1['refinementContractVersion'];
}>;
...
export class PostgresImageArtifactStore {
  ...
  async persistFinal(
    ...
    // generic Artifact lineage, so each Fashion producer gets one exact static
    // INSERT shape while CROP/RESIZE/etc. continue to name only generic columns.
    // F5 therefore fails closed if Fashion migration 032 is absent without making
    // generic or F4 persistence depend on the refinement schema.
  ...
  private async assertStoredFashionLineage(row: any, scope: AuthenticatedScope & { projectId: string }): Promise<void> {
    if (row.producer_operation === 'GARMENT_APPEARANCE_REFINEMENT') {
      ...
      if (
        ...
        || !lineage.refinementContractVersion
      ) throw new Error('Canonical Fashion refinement FINAL durable lineage is incomplete');

      // Reuse the accepted F4 durable authority instead of duplicating its layer/
      // source proof here. Read validation deliberately does not require the
      // Project cursor to remain current: after explicit Accept this row is a
      // historical canonical Artifact and must remain readable. Stale-source
      // currentness is enforced at F5 INSERT and Project Accept instead.
      const parent = await this.loadSource(lineage.refinementParentImageStorageId, scope);
      const parentHash = parent ? createHash('sha256').update(parent.bytes).digest('hex') : undefined;
      if (
        !parent
        || parent.role !== 'COMPOSITE'
        || parent.lifecycle !== 'FINAL'
        || parent.producerOperation !== 'GARMENT_TEXTURE_COMPOSITE'
        || parent.sourceImageStorageId !== lineage.sourceImageStorageId
        || parent.width !== Number(row.width)
        || parent.height !== Number(row.height)
        || parentHash !== lineage.refinementParentImageSha256
      ) throw new Error('Canonical Fashion refinement FINAL deterministic parent evidence is unavailable or inconsistent');
      return;
    }
...
function normalizeLineage(value: FinalImageLineage): NormalizedFinalImageLineage {
  ...
  if (value.producerOperation === 'GARMENT_APPEARANCE_REFINEMENT') {
    return normalizeGarmentAppearanceRefinementFinalLineage({
      ...value,
      sourceImageStorageId,
    });
  }
...
> {
  ...
  const refinementParentImageSha256 = row.refinement_parent_image_sha256 == null ? undefined : String(row.refinement_parent_image_sha256).trim();
  ...
  const hasRefinementFields = Boolean(
    refinementParentImageStorageId || refinementParentImageSha256 || refinementProfile || refinementContractVersion,
  );

  if (!producerOperation) {
    if (
      sourceImageStorageId || maskStorageId || garmentWarpLayerId || garmentWarpLayerSha256
      || producerParametersRaw || producerParametersSha256 || hasRefinementFields
  ...
  if (producerOperation === 'GARMENT_APPEARANCE_REFINEMENT') {
    if (
      !sourceImageStorageId || maskStorageId || garmentWarpLayerId || garmentWarpLayerSha256
      || producerParametersRaw || producerParametersSha256 || !refinementParentImageStorageId
      || !refinementParentImageSha256 || !refinementProfile || !refinementContractVersion
    ) throw new Error('Canonical Fashion refinement FINAL durable lineage is incomplete');
    return normalizeGarmentAppearanceRefinementFinalLineage({
      sourceImageStorageId,
      producerOperation,
  ...
  if (garmentWarpLayerId || garmentWarpLayerSha256 || producerParametersRaw || producerParametersSha256 || hasRefinementFields) {
