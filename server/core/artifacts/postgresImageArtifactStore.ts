...
export class PostgresImageArtifactStore {
  ...
  async persistFinal(scope: AuthenticatedScope & { projectId: string }, executionId: string, operationId: string, image: PixelImage): Promise<StoredFinalImage> {
    ...
    const result = await this.pool.query(`INSERT INTO canonical_image_artifacts
      ...
      RETURNING storage_id, image_bytes`, [storageId, scope.tenantId, scope.userId, scope.projectId, executionId, operationId, image.width, image.height, bytes]);
    return Object.freeze({ storageId: result.rows[0].storage_id, ...scope, executionId, operationId, role: 'COMPOSITE', lifecycle: 'FINAL', width: image.width, height: image.height, encoding: 'PNG_RGBA8_LOSSLESS', contentType: 'image/png', bytes: new Uint8Array(result.rows[0].image_bytes) });
  }
