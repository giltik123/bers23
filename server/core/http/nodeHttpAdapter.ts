  ...
  return async (request: IncomingMessage, response: ServerResponse): Promise<void> => {
    ...
    try {
      ...
      if (path === '/api/core/artifacts/masks' && request.method === 'POST') {
        ...
        const scope = authenticatedProjectScope(principal,projectId); const stored = await input.artifacts.masks.persist(scope, width, height, alpha); const artifactId = input.artifacts.external.issueStoredMask(stored.storageId, scope);
