...
/**
 * Intent-only browser transport for manual PARAMETRIC Garment acquisition.
 *
 * Stable Garment identity, optimistic revision and explicit contour are the only
 * browser-controlled fields. Current source/basis identity, hashes, persistence
 * identity, producer/validator provenance and replay remain Core-owned.
 */
export function createManualParametricGarmentAdmissionHttpAdapter(input: AdapterInput) {
  return async (request: IncomingMessage, response: ServerResponse): Promise<boolean> => {
    ...
    try {
      ...
      send(response, result.replayed ? 200 : 201, publicResult(result, garmentId));
      return true;
    } catch (cause) {
...
function publicResult(value: ManualParametricGarmentAdmissionResult, requestedGarmentId: string) {
  const representation = value.representation;
  const normalizedGarmentId = requestedGarmentId.toLowerCase();
  if (
    representation.garmentId !== normalizedGarmentId
...
function exactBody(value: unknown): AdmissionRequestBody {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw httpError(400, 'invalid_manual_parametric_request', 'Manual PARAMETRIC admission request must be a JSON object');
  }
