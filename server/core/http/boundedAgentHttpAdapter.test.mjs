...
test('bounded Agent HTTP start is CSRF protected, strict-field allowlisted and principal-normalized', async () => {
  ...
  const adapter = createBoundedAgentHttpAdapter({ workflow, auth: auth(), config });
...
test('resume/result/retry/cancel keep execution identity in the route and reject extra browser authority', async () => {
  const calls = [];
  const workflow = {
  ...
  await withServer(adapter, async base => {
    ...
    assert.equal(submitted.status, 200); assert.equal((await submitted.json()).terminalArtifactId, 'final-resize-artifact');
