...
export function createCreativeExecuteHandler(service: CreativeExecutionService) {
  return async (request: CoreRequest): Promise<CoreResponse> => {
    ...
    try {
      ...
      return { status: outcome.status === 'UNKNOWN' ? 202 : 200, body: publicCreativeOutcome(outcome, correlationId) };
    } catch (cause) {
...
export function publicCreativeOutcome(outcome: Awaited<ReturnType<CreativeExecutionService['execute']>>, correlationId?: string) {
  ...
  return { executionId: outcome.executionId, ...(correlationId ? { correlationId } : {}), status: outcome.status, imageUrl: typeof image?.value === 'object' && image?.value ? (image.value as { url?: string }).url : undefined, finalArtifactId: image?.role === 'COMPOSITE' && image.state === 'FINAL' ? image.id : undefined, artifacts: outcome.artifacts.map((artifact) => ({ id: artifact.id, kind: artifact.kind, role: artifact.role, url: typeof artifact.value === 'object' && artifact.value ? (artifact.value as { url?: string }).url : undefined, state: artifact.state, parentArtifactIds: artifact.metadata?.parentArtifactIds })), verification: outcome.verification };
}
