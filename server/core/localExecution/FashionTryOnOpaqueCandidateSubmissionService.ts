...
import type { FashionTryOnOpaqueTerminalReplayGuard } from './FashionTryOnOpaqueTerminalReplayGuard.ts';
...
type TerminalReplay = Pick<FashionTryOnOpaqueTerminalReplayGuard, 'tryRecoverGarmentWarp' | 'tryRecoverTextureComposite'>;
 ...
 *
 * Ordinary response-loss retries first consult the read-only terminal replay
 * guard. SUCCESS is returned only when the committed concrete mesh layer / FINAL
 * still matches the durable ticket lineage; FAILED follows the durable terminal
 * status. If finalization races upload/submit, the guard is consulted exactly
 * once more. A still non-terminal or invalid replay never converts the original
 * candidate failure into a false success.
 *
...
export class FashionTryOnOpaqueCandidateSubmissionService {
  ...
  async submitGarmentWarpCandidate(
    ...
    const lookup = Object.freeze({ ticketId: ticket.ticketId, projectId: ticket.scope.projectId });
    const terminal = await this.dependencies.terminalReplay.tryRecoverGarmentWarp(lookup, auth);
    ...
    try {
      ...
      const raced = await this.dependencies.terminalReplay.tryRecoverGarmentWarp(lookup, auth).catch(() => undefined);
  ...
  async submitTextureCompositeCandidate(
    ...
    const lookup = Object.freeze({ ticketId: ticket.ticketId, projectId: ticket.scope.projectId });
    const terminal = await this.dependencies.terminalReplay.tryRecoverTextureComposite(lookup, auth);
    if (terminal) return terminal;
    ...
    try {
      ...
      const raced = await this.dependencies.terminalReplay.tryRecoverTextureComposite(lookup, auth).catch(() => undefined);
      if (raced) return raced;
      throw cause;
    }
  }
