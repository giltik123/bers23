...
export async function checkLocalExecutionUploadSchema(pool: Pool): Promise<void> {
  ...
  if (!result.rows[0]?.table_name) throw new Error('local execution upload schema is incomplete; apply migration 006_local_execution_uploads.sql');
}
...
export async function migrateLocalExecutionUploadSchema(pool: Pool): Promise<void> {
  ...
  await pool.query(await readFile(new URL('./migrations/006_local_execution_uploads.sql', import.meta.url), 'utf8'));
}
