...
export class PostgresProjectStore {
  ...
  async navigate(scope:AuthenticatedScope,id:string,direction:'undo'|'redo'|'original'){
    return this.mutate(scope,id,async(client,project)=>{
      ...
      const target=(await client.query(`SELECT h.history_id,h.image_storage_id,a.width,a.height FROM canonical_project_history h JOIN canonical_image_artifacts a ON a.storage_id::text=h.image_storage_id AND a.tenant_id=h.tenant_id AND a.user_id=h.user_id AND a.project_id=h.project_id WHERE h.project_id=$1 AND h.tenant_id=$2 AND h.user_id=$3 AND h.retired_at IS NULL AND a.revoked_at IS NULL AND a.deleted_at IS NULL AND ((a.role='ORIGINAL' AND a.lifecycle='IMMUTABLE') OR (a.role='COMPOSITE' AND a.lifecycle='FINAL')) AND ${clause} LIMIT 1`,params)).rows[0];
