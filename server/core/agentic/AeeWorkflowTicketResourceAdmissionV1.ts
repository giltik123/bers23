...
/**
 * #548 resource admission at the last server-owned seam before a new local
 * execution ticket is issued.
 *
...
export class AeeWorkflowTicketResourceAdmissionV1 implements WorkflowBoundLocalExecutionTicketV2IssueGuard {
  ...
  async beforeIssue(input: LocalExecutionTicketIssueRequestV2): Promise<void> {
    ...
    assertIssueMatchesNode(input, snapshot, graph, nodeIndex);
...
function assertIssueMatchesNode(
  ...
  snapshot: Awaited<ReturnType<ContinuationReader['get']>> & {},
  graph: NonNullable<Awaited<ReturnType<PlanReader['get']>>>['graph'],
  nodeIndex: number,
): void {
  const node = graph.nodes[nodeIndex];
  ...
  if (input.inputs.length !== 1 || node.inputs.length !== 1 || node.inputs[0].name !== 'source') {
  ...
  if (ticketInput.artifactId !== expectedSourceArtifactId || ticketInput.kind !== 'image' || ticketInput.role !== node.inputs[0].artifactRole) {
  ...
  if (output.kind !== node.output.kind
...
function sourceArtifactIdFor(
  snapshot: NonNullable<Awaited<ReturnType<ContinuationReader['get']>>>,
  graph: NonNullable<Awaited<ReturnType<PlanReader['get']>>>['graph'],
  nodeIndex: number,
): string {
  const node = graph.nodes[nodeIndex];
