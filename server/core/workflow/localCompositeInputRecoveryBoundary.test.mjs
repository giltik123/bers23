...
test('C2a.1 production recovery wiring is workflow-owned, PostgreSQL-backed and canonical-artifact read-only', async () => {
  ...
  assert.match(delivery, /requireImmutableRoot\(snapshot\)/);
  assert.match(delivery, /requireCompletedSegmentMask\(snapshot\)/);
  assert.match(delivery, /ticket\.cost\.providerCalls !== 0/);
  assert.match(delivery, /ticket\.cost\.paidCloudCredits !== 0/);
  ...
  assert.doesNotMatch(delivery, /providerId|Billing|financialAccounts|persistFinal|persistMask|submitLocalResult|\.issue\(/);
});
