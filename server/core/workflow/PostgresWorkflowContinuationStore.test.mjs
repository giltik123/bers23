...
const rootInput = token => Object.freeze({
  artifactId: `${token}-source`, kind: 'image', role: 'WORKING', sha256: 'b'.repeat(64),
  parentArtifactIds: Object.freeze([`${token}-parent-b`, `${token}-parent-a`]),
});
const createInput = token => Object.freeze({
  ...
  inputArtifacts: Object.freeze([rootInput(token)]),
});
...
const ticketInputs = snapshot => snapshot.inputArtifacts.map(input => ({
  artifactId: input.artifactId, kind: input.kind, role: input.role, sha256: input.sha256,
}));
...
class FakePool {
  ...
  addTicket(snapshot, binding, overrides = {}) {
    const { ticket_json: ticketJsonOverrides = {}, ...rowOverrides } = overrides;
    this.tickets.set(binding.ticketId, {
      ...
      ticket_json: {
        version: binding.ticketVersion, nonce: binding.nonce, expiresAt: Date.parse(binding.expiresAt), policy: 'LOCAL_ONLY',
        cost: { providerCalls: 0, paidCloudCredits: 0 }, inputs: ticketInputs(snapshot), ...ticketJsonOverrides,
      },
      consumed_at: null, finalized_status: null, ...rowOverrides,
    });
  }
  ...
  async #query(sql, params) {
    ...
    if (sql.includes('INSERT INTO workflow_continuations')) {
      const [executionId, clientRequestId, tenantId, userId, projectId, planId, planRevision, planDigest, inputArtifactsJson] = params;
      ...
      const row = {
        execution_id: executionId, client_request_id: clientRequestId, tenant_id: tenantId, user_id: userId, project_id: projectId,
        plan_id: planId, plan_revision: planRevision, plan_digest: planDigest, input_artifacts_json: JSON.parse(inputArtifactsJson),
        state: 'READY', current_step_id: null, outstanding_ticket_id: null, outstanding_ticket_version: null,
        outstanding_ticket_nonce: null, outstanding_ticket_expires_at: null, completed_steps_json: [], terminal_artifact_id: null,
        failure_code: null, revision: '0', created_at: at, updated_at: at,
      };
    ...
    if (sql.includes('UPDATE workflow_continuations SET')) {
      ...
      Object.assign(row, {
        state, current_step_id: currentStepId, outstanding_ticket_id: ticketId, outstanding_ticket_version: ticketVersion,
        outstanding_ticket_nonce: nonce, outstanding_ticket_expires_at: expiresAt ? new Date(expiresAt) : null,
        completed_steps_json: JSON.parse(completedJson), terminal_artifact_id: terminalArtifactId, failure_code: failureCode,
        revision: String(expectedRevision + 1), updated_at: new Date(NOW + (expectedRevision + 1) * 1000),
      });
...
test('continuation persists immutable root Artifact authority and reconciles exact scoped replay', async () => {
  ...
  assert.deepEqual(created.inputArtifacts, [{ ...rootInput(token), parentArtifactIds: [`${token}-parent-a`, `${token}-parent-b`] }]);
  ...
  await assert.rejects(() => firstStore.create({
    ...createInput(token), inputArtifacts: [{ ...rootInput(token), sha256: 'c'.repeat(64) }],
  }), /already bound to another workflow continuation/, 'root integrity cannot be rebound under the same scoped request');

  const foreignInputTicket = ticket(`${token}-foreign-input`);
  pool.addTicket(created, foreignInputTicket, { ticket_json: { inputs: [{ artifactId: `${token}-foreign-artifact`, kind: 'image', role: 'WORKING', sha256: 'b'.repeat(64) }] } });
  await assert.rejects(() => firstStore.waitForLocalResult({ executionId: created.executionId, scope: created.scope, expectedRevision: 0, ticket: foreignInputTicket }), /not bound to a workflow root or completed dependency/);

  const wrongHashTicket = ticket(`${token}-wrong-hash`);
  pool.addTicket(created, wrongHashTicket, { ticket_json: { inputs: [{ artifactId: created.inputArtifacts[0].artifactId, kind: 'image', role: 'WORKING', sha256: 'c'.repeat(64) }] } });
  await assert.rejects(() => firstStore.waitForLocalResult({ executionId: created.executionId, scope: created.scope, expectedRevision: 0, ticket: wrongHashTicket }), /root input integrity/);
  ...
  assert.deepEqual(await afterRestart.get(created.executionId, created.scope), waiting, 'new Core store instance must recover exact durable waiting state and roots');
