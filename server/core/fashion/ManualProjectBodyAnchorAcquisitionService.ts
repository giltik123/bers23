...
export class ManualProjectBodyAnchorAcquisitionService {
  ...
  async acquire(auth: AuthenticatedScope, command: ManualProjectBodyAnchorAcquisitionCommand): Promise<ManualProjectBodyAnchorAcquisitionResult> {
    ...
    const anchorSet = await this.dependencies.bodyAnchors.createForExpectedImage(owner, normalized.projectId, Object.freeze({
      storageId: source.storageId, sha256: source.sha256, width: source.width, height: source.height,
    }), Object.freeze({ payload, producerId: MANUAL_BODY_ANCHOR_PRODUCER_ID, producerVersion: MANUAL_BODY_ANCHOR_PRODUCER_VERSION }));
    assertPersistedAuthority(anchorSet, source, normalized.projectId, expectedPayloadSha256);
    return Object.freeze({ projectId: normalized.projectId, sourceArtifactId: normalized.sourceArtifactId, anchorSet });
  }
}

function assertPersistedAuthority(anchorSet: ManagedProjectBodyAnchorSet, source: StoredProjectImageEvidence, projectId: string, expectedPayloadSha256: string): void {
  let returnedPayloadSha256: string | undefined;
  try { returnedPayloadSha256 = bodyAnchorPayloadSha256(anchorSet.payload); } catch { returnedPayloadSha256 = undefined; }
  ...
  if (!matches) throw acquisitionError(409, 'body_anchor_acquisition_authority_mismatch', 'Persisted body-anchor evidence does not match the exact resolved Project source, payload and server-owned provenance');
}

function normalizeCommand(command: ManualProjectBodyAnchorAcquisitionCommand): ManualProjectBodyAnchorAcquisitionCommand {
  if (!command || typeof command !== 'object') throw acquisitionError(400, 'invalid_manual_body_anchor_request', 'Manual body-anchor acquisition request must be an object');
  if (typeof command.projectId !== 'string' || !UUID_PATTERN.test(command.projectId)) throw acquisitionError(400, 'invalid_manual_body_anchor_request', 'projectId must be a UUID');
  if (typeof command.sourceArtifactId !== 'string') throw acquisitionError(400, 'invalid_manual_body_anchor_request', 'sourceArtifactId must be a string');
  const sourceArtifactId = command.sourceArtifactId.trim();
  if (!sourceArtifactId || sourceArtifactId.length > MAX_SOURCE_ARTIFACT_ID_LENGTH || /[\u0000-\u001f\u007f]/u.test(sourceArtifactId)) {
  ...
  return Object.freeze({ projectId: command.projectId.toLowerCase(), sourceArtifactId, payload: command.payload });
}

function assertResolvedProjectImage(source: StoredProjectImageEvidence, projectId: string): void {
  if (source.projectId !== projectId || !source.storageId || !/^[0-9a-f]{64}$/.test(source.sha256)
    || !Number.isSafeInteger(source.width) || source.width < 1 || !Number.isSafeInteger(source.height) || source.height < 1
