...
-- F5 lineage is an additive Fashion-only extension. Generic and existing F4
-- persistence remain composable without this migration.
...
-- Extend the already-closed generic/F4 lineage shape without making any
-- existing producer depend on F5-specific columns.
ALTER TABLE canonical_image_artifacts
...
ALTER TABLE canonical_image_artifacts
  ADD CONSTRAINT canonical_image_artifacts_lineage_shape_check CHECK (
    (
      ...
      AND refinement_parent_image_storage_id IS NULL
      AND refinement_parent_image_sha256 IS NULL
    ...
    OR (
      ...
      AND producer_parameters IS NULL
      AND producer_parameters_sha256 IS NULL
      AND refinement_parent_image_storage_id IS NOT NULL
      AND refinement_parent_image_sha256 IS NOT NULL
...
BEGIN
  ...
  FOR SHARE OF p, source_image, parent_image;
...
CREATE TRIGGER canonical_image_artifacts_fashion_refinement_immut_guard
  BEFORE UPDATE OF producer_operation,source_image_storage_id,mask_storage_id,garment_warp_layer_id,garment_warp_layer_sha256,producer_parameters,producer_parameters_sha256,refinement_parent_image_storage_id,refinement_parent_image_sha256,refinement_profile,refinement_contract_version
