...
export class PostgresGarmentStore {
  ...
  async appendView(scope: GarmentOwnerScope, garmentId: string, expectedRevision: number, input: GarmentImageInput, limits: GarmentImageLimits): Promise<ManagedGarment> {
    ...
    const preflight = await this.pool.query(`SELECT revision FROM canonical_garments
      WHERE garment_id=$1 AND tenant_id=$2 AND user_id=$3 AND deleted_at IS NULL`, [garmentId, scope.tenantId, scope.userId]);
    ...
    try {
      ...
      const ordinalResult = await client.query(`SELECT COALESCE(MAX(ordinal), -1) + 1 AS ordinal
        FROM canonical_garment_views WHERE garment_id=$1 AND tenant_id=$2 AND user_id=$3`,
      [garmentId, scope.tenantId, scope.userId]);
      const ordinal = Number(ordinalResult.rows[0]?.ordinal);
      if (!Number.isSafeInteger(ordinal) || ordinal < 1) throw new Error('Managed garment next-view ordinal is invalid');

      await insertManagedView(client, scope, garmentId, viewId, ordinal, normalized);
      const updated = await client.query(`UPDATE canonical_garments SET revision=revision+1, updated_at=CURRENT_TIMESTAMP
        WHERE garment_id=$1 AND tenant_id=$2 AND user_id=$3 AND deleted_at IS NULL AND revision=$4 RETURNING revision`,
      [garmentId, scope.tenantId, scope.userId, expectedRevision]);
  ...
  async get(scope: GarmentOwnerScope, garmentId: string): Promise<ManagedGarment | undefined> {
    ...
    const result = await this.pool.query(`${GARMENT_SELECT}
      WHERE g.garment_id=$1 AND g.tenant_id=$2 AND g.user_id=$3 AND g.deleted_at IS NULL`, [garmentId, scope.tenantId, scope.userId]);
...
const GARMENT_SELECT = `SELECT g.*,
  COALESCE((SELECT jsonb_agg(jsonb_build_object(
    'id',v.view_id,'ordinal',v.ordinal,'kind',v.view_kind,'sourceContentType',v.source_content_type,
    'width',v.width,'height',v.height,'encoding',v.encoding,'contentType',v.content_type,
    'contentSha256',v.content_sha256,'storageBackend',v.storage_backend,'createdAt',v.created_at
  ) ORDER BY v.ordinal)
  FROM canonical_garment_views v
...
async function normalizeGarmentImage(input: GarmentImageInput, limits: GarmentImageLimits): Promise<NormalizedGarmentImage> {
  ...
  try {
    ...
    if (decodedContentType !== sourceContentType) throw httpError(415, 'garment_image_media_type_mismatch', 'Garment Content-Type does not match the decoded image format');
    if ((metadata.pages ?? 1) !== 1) throw httpError(400, 'animated_garment_image_unsupported', 'Animated or multi-page garment images are not supported');
    normalized = await decoder.rotate().toColourspace('srgb').ensureAlpha().raw().toBuffer({ resolveWithObject: true });
  } catch (error) {
    if (isHttpError(error)) throw error;
    throw httpError(400, 'invalid_garment_image', 'Garment image is malformed or unsafe');
  }
  const width = Number(normalized.info.width); const height = Number(normalized.info.height);
  ...
  if (png.byteLength > limits.maxUploadBytes) throw httpError(413, 'normalized_garment_image_too_large', 'Canonical garment image exceeds the configured storage limit');
  return Object.freeze({ viewKind, sourceContentType, width, height, contentSha256: createHash('sha256').update(png).digest('hex'), png });
}

async function insertManagedView(client: PoolClient, scope: GarmentOwnerScope, garmentId: string, viewId: string, ordinal: number, image: NormalizedGarmentImage): Promise<void> {
...
function fromGarmentRow(row: any): ManagedGarment {
  const views = (Array.isArray(row.views) ? row.views : []).map((view: any) => Object.freeze({
    id: String(view.id), ordinal: Number(view.ordinal), kind: normalizeViewKind(view.kind), sourceContentType: normalizeSourceContentType(view.sourceContentType),
    width: Number(view.width), height: Number(view.height), encoding: 'PNG_RGBA8_LOSSLESS' as const, contentType: 'image/png' as const,
    contentSha256: String(view.contentSha256), storageBackend: normalizeStorageBackend(view.storageBackend), createdAt: new Date(view.createdAt).toISOString(),
  }));
  const primaryViewId = String(row.primary_view_id ?? '');
  if (!primaryViewId || views.length < 1 || !views.some((view) => view.id === primaryViewId)) throw new Error('Managed garment primary-view invariant is violated');
  return Object.freeze({ id: String(row.garment_id), name: String(row.name), representationTier: row.representation_tier, status: row.status,
    revision: Number(row.revision), primaryViewId, views: Object.freeze(views), createdAt: new Date(row.created_at).toISOString(), updatedAt: new Date(row.updated_at).toISOString() });
}

function revisionConflict(): Error & { status: number; code: string } { return httpError(412, 'garment_revision_conflict', 'Garment revision changed; reload the aggregate before appending another view'); }
function isUuid(value: unknown): value is string { return typeof value === 'string' && UUID_PATTERN.test(value); }
function normalizeName(value: string): string { const name = typeof value === 'string' ? value.trim() : ''; if (!name || name.length > 200) throw httpError(400, 'invalid_garment_name', 'Garment name must contain 1 to 200 characters'); return name; }
function normalizeViewKind(value: unknown): GarmentViewKind { const kind = typeof value === 'string' ? value.trim().toUpperCase() : ''; if (!(GARMENT_VIEW_KINDS as readonly string[]).includes(kind)) throw httpError(400, 'invalid_garment_view_kind', 'Garment view kind is unsupported'); return kind as GarmentViewKind; }
function normalizeSourceContentType(value: unknown): 'image/png' | 'image/jpeg' | 'image/webp' { if (value === 'image/png' || value === 'image/jpeg' || value === 'image/webp') return value; throw httpError(415, 'unsupported_media_type', 'Supported garment images are PNG, JPEG and WebP'); }
function decodedImageContentType(format: unknown): 'image/png' | 'image/jpeg' | 'image/webp' { if (format === 'png') return 'image/png'; if (format === 'jpeg') return 'image/jpeg'; if (format === 'webp') return 'image/webp'; throw httpError(415, 'unsupported_media_type', 'Decoded garment image format is unsupported'); }
function normalizeStorageBackend(value: unknown): 'POSTGRES_BYTEA_V1' { if (value === 'POSTGRES_BYTEA_V1') return value; throw new Error('Managed garment storage provenance is unsupported'); }
function isHttpError(value: unknown): value is Error & { status: number; code: string } { return Boolean(value && typeof value === 'object' && Number.isInteger((value as any).status) && typeof (value as any).code === 'string'); }
