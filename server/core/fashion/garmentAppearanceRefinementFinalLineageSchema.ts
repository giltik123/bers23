...
export async function checkGarmentAppearanceRefinementFinalLineageSchema(pool: Pool): Promise<void> {
  ...
  if (!shape || shape.contype !== 'c' || !shape.convalidated
    || !shapeDef.includes("producer_operation = 'GARMENT_APPEARANCE_REFINEMENT'::text")
    || !shapeDef.includes('source_image_storage_id IS NOT NULL')
  ...
  if (triggerMap.size !== 2 || Number(insert?.tgtype) !== BEFORE_INSERT_ROW_TGTYPE || insert?.tgenabled !== 'O'
    || insert?.proname !== 'canonical_assert_fashion_refinement_final_insert'
    || Number(immutable?.tgtype) !== BEFORE_UPDATE_ROW_TGTYPE || immutable?.tgenabled !== 'O'
